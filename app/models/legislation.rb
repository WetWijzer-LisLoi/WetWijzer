...
#  is_empty_content    :integer          default(0)
#  is_modification     :integer          default(0)
#  translation_missing :integer          default(0)
#  senate              :text             default("N/A")
#
# Indexes
#
...
class Legislation < ReadonlyRecord
  ...
  # Returns the clean display title, preferring short_title if populated
  # @return [String] the cleaned title for UI display
  def display_title
    short_title.presence || self.class.generate_short_title(title)
  end

  # Generates a clean display title by stripping editorial metadata
  # from Belgian Staatsblad titles (NOTA blocks, amendment refs, etc.)
  # @param full_title [String] the raw legislation title
  # @return [String] the cleaned title
  def self.generate_short_title(full_title)
    return full_title if full_title.blank?

    title = full_title.dup
    # 1. Strip (NOTA : ...) and (NOTA 1 : ...) blocks
    title.gsub!(/\s*\(NOTA\s*\d*\s*:.*?\)/mi, '')
    # 2. Strip [NOTA : ...] blocks
    title.gsub!(/\s*\[NOTA\s*:.*?\]/mi, '')
    # 3. Strip <angle-bracket amendment refs> e.g. <DWG2008-12-05/75, art. 98>
    title.gsub!(/\s*<[^>]+>/m, '')
    # 4. Strip trailing (Inwerkingtreding : ...) if at end
    title.gsub!(/\s*\(\s*Inwerkingtreding\s*:.*?\)\s*$/i, '')
    # 5. Strip (Entrée en vigueur : ...) French equivalent
    title.gsub!(/\s*\(\s*Entr[ée]e en vigueur\s*:.*?\)\s*$/i, '')
    # 6. Clean up multiple spaces and trailing punctuation artifacts
    title.strip!
    title.squeeze!(' ')
    title
  end
