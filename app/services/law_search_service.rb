...
class LawSearchService
  ...
  def self.perform_article_text_search_flexible(scope, title, params)
    ...
    # Build OR condition for each token in article text (and optionally tags)
  ...
  def self.perform_title_search_flexible(scope, title, params)
    ...
    # Build OR condition for each token in title (and optionally tags)
    conditions = if search_tags
                   tokens.map { '(LOWER(title) LIKE ? OR LOWER(tags) LIKE ?)' }.join(' OR ')
                 else
                   tokens.map { 'LOWER(title) LIKE ?' }.join(' OR ')
                 end
  ...
  def self.apply_sort(scope, params)
    ...
    order_clause = SORT_OPTIONS[sort_param] || SORT_OPTIONS[DEFAULT_SORT]

    tag_ranked_scope = apply_tag_prioritization(scope, params, order_clause)
    return tag_ranked_scope if tag_ranked_scope

    scope.order(Arel.sql(order_clause))
  end

  def self.apply_tag_prioritization(scope, params, order_clause)
    title = params[:title]
    prioritize_tags = !params.key?(:prioritize_tags) || params[:prioritize_tags] == '1'
    return nil unless title.present? && prioritize_tags

    term = normalize_query(title)
    tokens = extract_tokens(term)
    return nil if tokens.empty?

    rank_sql = build_tag_rank_clause(tokens)
    scope.order(Arel.sql("#{rank_sql} ASC")).order(Arel.sql(order_clause))
  end
  private_class_method :apply_sort, :apply_tag_prioritization
  ...
  def self.build_tag_rank_clause(tokens)
    ...
    ors = tokens.map do |t|
      "(LOWER(tags) LIKE '%\"#{t} %' OR LOWER(tags) LIKE '% #{t} %' OR LOWER(tags) LIKE '% #{t}\"%' OR LOWER(tags) LIKE '%\"#{t}.%')"
    end.join(' OR ')
