...
class LawSearchService
  ...
  def self.by_text_or_title(scope, params)
    ...
    if cleaned_query.length == 10
      return scope.where('numac LIKE ?', "%#{sanitize_sql_like(cleaned_query)}%")
    end
  ...
  def self.perform_title_search_flexible(scope, title, params)
    ...
    # Each token must be present in either title or tags
    conditions = if search_tags
                   tokens.map { '(LOWER(title) LIKE ? OR LOWER(tags) LIKE ?)' }.join(' AND ')
                 else
                   tokens.map { 'LOWER(title) LIKE ?' }.join(' AND ')
                 end
