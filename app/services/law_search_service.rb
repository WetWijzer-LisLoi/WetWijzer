...
class LawSearchService
  ...
  def self.by_language(scope, params)
    ...
    language_ids << '3' if include_german?(params)
  ...
  # Checks if German documents should be included
  # @param params [Hash] The search parameters
  ...
  def self.type_ids_from_params(params)
    ...
    # Add German document types if German is selected
    type_ids += selected_type_ids(:de, params) if include_german?(params)
  ...
  def self.find_article_text_matches(term, _scope, sqlite)
    ...
    if sqlite
      # Use n-grams if the table exists, with a hard LIMIT to prevent runaway queries
      grams = build_ngrams(term)
      if grams.any?
        placeholders = grams.map { '?' }.join(', ')
        sql = <<~SQL.squish
          SELECT DISTINCT a.content_numac, a.language_id
          FROM articles a
          INNER JOIN articles_text_ngrams ng ON ng.rowid = a.rowid
          WHERE ng.gram IN (#{placeholders})
          GROUP BY a.rowid
          HAVING COUNT(DISTINCT ng.gram) >= ?
            AND LOWER(a.article_text) LIKE ?
          LIMIT 500
        SQL
        conn.select_rows(sql, 'ArticleTextNgram', grams + [grams.size, like_pattern])
      else
        # Short query: fall back to simple LIKE with limit
        sql = <<~SQL.squish
