...
class LawSearchService
  ...
  # Justel's production corpus is canonical only in Dutch and French.
  # language_id=3 is reserved for a future provenance-checked German corpus
  # and must remain invisible until that importer has been rebuilt.
  ...
  def self.search(params)
    ...
    ensure_sqlite_timeout if sqlite_adapter?

    scope = Legislation.where(language_id: CANONICAL_LANGUAGE_IDS)
  ...
  def self.by_language(scope, params)
    ...
    language_ids << '2' if include_french?(params) || include_german?(params)
  ...
  def self.language_filtered?(params)
    ...
    params[:lang_nl].present? || params[:lang_fr].present? || params[:lang_de].present?
  end
  ...
  # Compatibility alias for old URLs and saved searches. Until a verified
  # German source corpus exists, a German selection searches canonical French
  # source text and the UI/answer layer remains responsible for German output.
  # @param params [Hash] The search parameters
  # @return [Boolean] true if German documents should be included
  def self.include_german?(params)
    params[:lang_de] == '1'
  end
  private_class_method :include_german?
  ...
  def self.parse_date(date_string)
    ...
    if date_string.match?(%r{\A\d{1,2}/\d{1,2}/\d{4}\z})
      ...
      begin
        ...
        Rails.logger.warn("[LawSearch] Operation failed: #{e.message}")
        nil
      end
  ...
  def self.type_ids_from_params(params)
    ...
    # Old German filters currently fall back to canonical French source types.
    type_ids += selected_type_ids(:fr, params) if include_german?(params)
  ...
  def self.perform_article_text_search_exact(scope, title, params)
    ...
    # Strategy: collect matching numac/language_id pairs via direct SQL to avoid
    # the pathological rowid IN (subquery) pattern that causes 504 timeouts.
    # Use n-grams when available, fall back to LIKE.
    matching_pairs = find_article_text_matches(term, scope, sqlite)

    # Also match via tags if enabled
    if search_tags
      tag_sql, binds = build_tags_predicate_and_binds(title)
      tag_matches = scope.where("(tags IS NOT NULL AND (#{tag_sql}))", *binds)
      result = if matching_pairs.any?
                 scope.where(numac: matching_pairs.map { |p| p[0] }).or(tag_matches).distinct
               else
                 tag_matches.distinct
               end
    elsif matching_pairs.any?
      result = scope.where(numac: matching_pairs.map { |p| p[0] }).distinct
    else
      result = scope.none
    end

    instrument_strategy(:search_in_text_exact, title_length: term.length, sqlite: sqlite,
                                               tag_tokens: extract_tokens(title).size)
    result
  end

  # Finds legislation numacs that have matching article text, using direct SQL
  # to avoid ActiveRecord subquery overhead.
  def self.find_article_text_matches(term, _scope, sqlite)
    like_pattern = "%#{sanitize_sql_like(term)}%"
    conn = ActiveRecord::Base.connection

    if sqlite
      # Narrow with the FTS index, verify with LIKE. This branch used to join
      # articles_text_ngrams, an auxiliary trigram index that made text search
      # silently return almost nothing for months.
      #
      # Correcting what this comment used to say: the backfill did NOT fail for
      # want of "~30-50 GB the host does not have". It ran, and it is the
      # reason the host has no space - measured 2026-08-15, the table plus its
      # primary-key autoindex and idx_articles_text_ngrams_gram hold ~930M rows
      # across roughly 43 GiB of the 54 GB laws database, for an index with no
      # reader left anywhere in the codebase. The daily refresh no longer tops
      # it up (see daily_rails_steps.sh); dropping it needs a VACUUM and is a
      # deliberate, owner-scheduled operation because the file cannot shrink in
      # place. The FTS5
      # index exists, is complete, and is rebuilt with the corpus; a phrase
      # MATCH finds every article containing the words in order, and the LIKE
      # keeps the exact-substring contract. The MATCH stays in its own
      # subquery: joining an FTS table directly lets the planner re-run the
      # search per row (measured at 26 minutes on one phrase).
      match_query = fts_phrase_query(term)
      if match_query
        sql = <<~SQL.squish
          SELECT DISTINCT a.content_numac, a.language_id
          FROM articles a
          WHERE a.rowid IN (SELECT rowid FROM articles_fts WHERE articles_fts MATCH ?)
            AND LOWER(a.article_text) LIKE ?
          LIMIT 500
        SQL
        conn.select_rows(sql, 'ArticleTextFts', [match_query, like_pattern])
      else
        # No tokenizable words (punctuation-only or too short): bounded LIKE.
        sql = <<~SQL.squish
          SELECT DISTINCT content_numac, language_id
          FROM articles
          WHERE LOWER(article_text) LIKE ?
          LIMIT 500
        SQL
        conn.select_rows(sql, 'ArticleTextLike', [like_pattern])
      end
    else
      sql = <<~SQL.squish
        SELECT DISTINCT content_numac, language_id
    ...
    Rails.logger.warn("Article text match failed: #{e.class}: #{e.message}")
    []
  end

  # A safely-quoted FTS5 phrase query for the term's words, or nil when the
  # term has no tokenizable words. Double quotes make it a phrase (words in
  # order) and neutralize FTS operator syntax in user input.
  def self.fts_phrase_query(term)
    words = term.to_s.scan(/[[:alnum:]]+/)
    return nil if words.empty?

    %("#{words.join(' ')}")
  end
  private_class_method :perform_article_text_search_exact, :find_article_text_matches,
                       :fts_phrase_query
  ...
  def self.perform_article_text_search_flexible(scope, title, params)
    ...
    # LIMIT 1 inside EXISTS prevents scanning all 2.8M articles per outer row.
    conditions = tokens.map do
      if search_tags
        '(EXISTS (SELECT 1 FROM articles WHERE articles.content_numac = legislation.numac AND articles.language_id = legislation.language_id AND LOWER(articles.article_text) LIKE ? LIMIT 1) OR LOWER(tags) LIKE ?)'
      else
        'EXISTS (SELECT 1 FROM articles WHERE articles.content_numac = legislation.numac AND articles.language_id = legislation.language_id AND LOWER(articles.article_text) LIKE ? LIMIT 1)'
      end
    end.join(' AND ')
  ...
  def self.perform_title_search_exact(scope, title, params)
    ...
    # Short-circuit: for simple queries (1-2 tokens), direct LIKE is faster
    # than the ngram join+group+having path (0.2s vs 4.2s on 480K rows).
    # The ngram approach only benefits multi-token queries where each token
    # can be independently filtered via the ngram index.
    rel = if sqlite && tokens.size > 2
            by_title_ngrams(scope, title)
          else
  ...
  # Combine title matches with tag matches by running TWO separate fast queries
  # and merging results in Ruby.
  #
  # CRITICAL: Never let SQLite evaluate `title LIKE ? OR tags LIKE ?` in a single
  # query – it forces a full 480K-row scan. Instead:
  #   1. Run the title/ngram query → collect rowids (fast, ~0.2s)
  #   2. Run the tag query ONLY if title returned few results (< 50)
  #      Tag LIKE scans 480K rows (~40s) and aren't worth it when title already found enough
  #   3. Merge in Ruby (instant)
  #   4. Fetch by literal rowid IN (...) (fast, indexed)
  TAG_SEARCH_THRESHOLD = 50 # Only run tag scan if title matches < this

  def self.combine_relations_by_rowid_or_tags(scope, rel, title, params)
    search_tags = !params.key?(:search_in_tags) || params[:search_in_tags] == '1'
    conn = ActiveRecord::Base.connection

    # Step 1: collect rowids from the title/LIKE relation (capped at 500)
    title_ids = conn.select_values(rel.select('legislation.rowid').limit(500).to_sql)

    # Step 2: collect rowids from tag matches – but ONLY if title returned few results.
    # Tag LIKE scans all 480K rows (~40s for common terms like 'grondwet').
    # When title already found 50+ matches, tag matches add marginal value.
    tag_ids = []
    if search_tags && title_ids.size < TAG_SEARCH_THRESHOLD
      tag_sql, binds = build_tags_predicate_and_binds(title)
      tag_scope = scope.where("(tags IS NOT NULL AND (#{tag_sql}))", *binds)
      tag_ids = conn.select_values(tag_scope.select('legislation.rowid').limit(500).to_sql)
    end

    # Step 3: merge IDs in Ruby (instant set union)
    all_ids = (title_ids + tag_ids).uniq

    # Step 4: fetch by literal rowid list (indexed, fast)
    result = if all_ids.any?
               scope.where("legislation.rowid IN (#{all_ids.map(&:to_i).join(',')})")
             else
               scope.none
             end

    instrument_strategy(:combine_relations, title_matches: title_ids.size,
                                            tag_matches: tag_ids.size,
                                            total_unique: all_ids.size,
                                            tags_skipped: search_tags && title_ids.size >= TAG_SEARCH_THRESHOLD)
    result
  end
  ...
  # Cache the ngram table check per-process – running COUNT(*) on every
  # search request adds ~100ms of overhead for no reason.
  def self.ngram_table_populated?
    return true unless sqlite_adapter?

    return @ngram_populated if defined?(@ngram_populated)

    @ngram_populated = begin
  ...
  # The by_article_ngrams / execute_article_ngram_search pair that used to
  # live here was removed on 2026-08-09: it was never dispatched from any
  # strategy chain, and the articles_text_ngrams trigram index it joined was
  # never built (the backfill needs ~30-50 GB the host does not have). Article
  # text search runs through find_article_text_matches, which narrows with the
  # complete articles_fts index instead.
  ...
  # Cached per-process since the adapter never changes at runtime.
  def self.sqlite_adapter?
    return @sqlite_adapter if defined?(@sqlite_adapter)

    @sqlite_adapter = ActiveRecord::Base.connection.adapter_name.to_s.downcase.start_with?('sqlite')
  end
  private_class_method :sqlite_adapter?

  # Set a busy_timeout on the SQLite connection to prevent queries from
  # blocking Puma workers indefinitely when the database is locked.
  def self.ensure_sqlite_timeout
    ActiveRecord::Base.connection.execute('PRAGMA busy_timeout = 15000') # 15s max wait
  rescue StandardError => e
    Rails.logger.warn("[LawSearch] Operation failed: #{e.message}")
    # Silently continue if PRAGMA not supported
  end
  private_class_method :ensure_sqlite_timeout
  ...
  def self.apply_sort(scope, params)
    ...
    return apply_relevance_sort(scope, params) if sort_param == 'relevance'
  ...
  def self.apply_relevance_sort(scope, params)
    ...
    # Core/popular laws appear first, then tag matches, then by relevance score (desc), then by date (desc)
    core_rank = 'CASE WHEN is_core = 1 THEN 0 ELSE 1 END'
    scope.order(Arel.sql("#{core_rank} ASC, #{tag_rank} ASC, #{relevance_score} DESC, date DESC"))
  end
