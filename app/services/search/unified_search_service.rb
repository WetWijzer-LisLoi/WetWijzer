...
module Search
  class UnifiedSearchService
    ...
    attr_reader :locale, :last_timings_ms, :failed_sources
    ...
    def search(query, sources: ADAPTERS.keys, filters: {}, limit_per_source: 5)
      ...
      sources.each do |source|
        ...
        begin
          ...
          # FBL-064: one dead corpus must not silence the healthy ones, and
          # must not look healthy either - callers get the failure metadata.
          @failed_sources[source] = e.class.name
          Rails.logger.error("[UnifiedSearch] #{source} adapter failed: #{e.class}")
        ensure
          @last_timings_ms[source] =
    ...
    def get_adapter(source)
      ...
      @adapters[source] ||= ADAPTERS[source].new(locale: locale)
    end
  end
end
