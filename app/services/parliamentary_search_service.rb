...
class ParliamentarySearchService
  ...
  class << self
    ...
    def search(query: nil, parliament: nil, year: nil, doc_type: nil, lang: nil,
               search_body: false, date_from: nil, date_to: nil,
               page: 1, per_page: DEFAULT_PER_PAGE, sort: 'date_desc')
      return SearchResult.new(results: [], total_count: 0) unless db_path_exists?

      db = connection
      conditions, bind_params = build_conditions(db, query, parliament, year, doc_type: doc_type,
                                                lang: lang, search_body: search_body,
    ...
    def build_conditions(db, query, parliament, year, doc_type: nil, lang: nil, search_body: false,
      ...
      if query.present?
        fts_expr = query.scan(/\p{Word}+/).map { |t| %("#{t}") }.join(' ')
        if fts5_available?(db) && fts_expr.present?
          # Quote each token so FTS5 punctuation (covid-19, art. 7, parentheses)
          # is literal text, not MATCH operators — raw input raised or returned 0.
          conditions << 'id IN (SELECT rowid FROM documents_fts WHERE documents_fts MATCH ?)'
          bind_params << fts_expr
        elsif search_body
      ...
      # document_date is a DATE column holding ISO text, so normalised ISO
      # bounds compare directly. date() keeps a value that carries a time part
      # from falling outside an inclusive upper bound.
      if (from_iso = Search::DateInput.iso(date_from))
        conditions << 'date(document_date) >= ?'
        bind_params << from_iso
      end

      if (to_iso = Search::DateInput.iso(date_to))
        conditions << 'date(document_date) <= ?'
        bind_params << to_iso
      end
