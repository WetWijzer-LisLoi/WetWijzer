    ...
    begin
      ...
      rows = db.execute(<<~SQL, [search_term, search_term, search_term, search_term])
        SELECT DISTINCT l.id, l.title_nl, l.title_fr, l.document_type,
               l.publication_date, l.effective_date, l.source_url, l.is_in_force
        FROM tax_articles a
      ...
      rows.map do |row|
        ...
        # Try publication_date first, then effective_date, then known-dates map
        pub_date = parse_date_safe(row['publication_date']) ||
                   parse_date_safe(row['effective_date']) ||
                   KNOWN_TAX_DATES[doc_type]
