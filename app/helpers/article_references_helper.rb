...
module ArticleReferencesHelper
  ...
  def extract_references_with_pattern(text, pattern, references)
    text.scan(pattern) do |matches|
      ...
      # e.g., "14" + "WAALS_GEWEST" => "14_waals_gewest"
      # e.g., "17" + "TOEKOMSTIG RECHT" => "17_toekomstig_recht"
      if pattern == special_article_pattern && matches_array.size >= 2
        # Normalize to use underscore separator and lowercase
        full_id = "#{matches_array[0]}_#{matches_array[1]}".gsub(/\s+/, '_').downcase.strip
        references << full_id
      else
  ...
  def structural_article_reference?(text, match)
    ...
    at_line_start?(preceding_text) && followed_by_period?(following_text)
  end
  ...
  def build_article_exdec_mapping(_main_law_numac, exdecs, language_id)
    ...
    # Using Hash[] or {}.merge creates a new hash without the default proc
    mapping.to_h
  end
