...
module ParliamentaryHelper
  ...
  def clean_jurisprudence_full_text(text, case_number: nil)
    ...
    # The dispositif opens the operative part of a judgment. Both the intro
    # guard and the heading detection below go through this one lambda, so the
    # two can never drift apart - which is how "OM DIE REDENEN," slipped
    # through: the detector knew only "OM DEZE REDENEN" and required the phrase
    # to end the line, so "PAR CES MOTIFS, DECIDE:" missed it too.
    dispositif_line = lambda do |s|
      # NB: no /x here. Extended mode ignores literal spaces, which silently
      # turned "PAR CES MOTIFS" into "PARCESMOTIFS" and matched nothing.
      keyword = /\A(?:PAR\s+CES\s+MOTIFS|OM\s+D(?:IE|EZE)\s+REDENEN|DOOR\s+DEZE\s+REDENEN|AUS\s+DIESEN\s+GR(?:U|\u00dc)NDEN|DISPOSITIF|BESCHIKKEND\s+GEDEELTE|BESLUIT|BESLISSING)\b/i
      next false unless s.match?(keyword)
      # The bare phrase, with or without trailing punctuation - exactly the
      # line the old detector accepted. Anchored at both ends on purpose: a
      # PDF-wrapped continuation line "Besluit van de Vlaamse Regering" starts
      # with a keyword too, and must stay prose.
      next true if s.match?(/\A(?:#{keyword.source[2..]})\s*[,:.]?\s*\z/i)
      # Or the phrase carrying its own clause ("PAR CES MOTIFS, DECIDE:"):
      # short and shouted. Counted with a regex, not String#count - the latter
      # reads a single-quoted set literally, so accented capitals scored as
      # lowercase and the guard measured something other than what it claimed.
      letters = s.gsub(/[^[:alpha:]]/, '')
      s.length <= 80 && !letters.empty? &&
        letters.scan(/[A-Z\u00c0-\u00d6\u00d8-\u00de]/).size.to_f / letters.length > 0.7
    end

    # A line that opens a numbered section or the dispositif is a heading, not
    # a masthead line. The intro block below consumes what it claims with
    # `next`, and emit_section_heading is the only thing that clears
    # intro_mode, so an intro that swallows the first heading keeps the WHOLE
    # judgment in masthead styling. Measured 2026-09-05 across 164 sampled
    # judgments: 42.7% were stuck that way, the worst with no body paragraph at
    # all - a reader saw dim little prose with mid-document lines centred bold
    # white. Kept deliberately narrow: a bare all-caps line is NOT listed here,
    # because that is what a court name in a real masthead looks like.
    section_start = lambda do |s|
      s.match?(/\A[IVXLC]+\.\s+\S/) ||        # "I. OBJET", "I.\tRECHTSPLEGING"
        s.match?(/\A[IVXLC]+\.\d+\.\s+\S/) || # "V.1. Thèse de la partie requérante"
        dispositif_line.call(s)
    end
    ...
    result.each_line do |line|
      ...
      # The recitals open the body of a judgment - "Vu la requete introduite le
      # ...", "Gelet op het bestreden arrest" - so the masthead is over, even in
      # the many Conseil d'Etat judgments that carry no numbered heading at all
      # (their header block is closed by a rule of dashes, which the cleaning
      # pass strips before this one runs). Without this their closing signature
      # line renders as a centred bold white court name.
      if intro_mode && stripped.match?(/\A(?:Vu\b|Gelet\s+op\b|Gezien\b)/i)
        # Flush BEFORE clearing, as emit_section_heading and the "Het Hof,"
        # branch do: the party block still sitting in current_para must go out
        # with masthead styling and its <br> joins, not be welded onto the
        # recital as one body paragraph.
        flush_para.call
        intro_mode = false
      end

      # section_start lines fall through to the detection below, which is what
      # ends the masthead; without that guard the intro swallows them forever.
      if intro_mode && !section_start.call(stripped)
      ...
      # Once the signature block has opened ("Ainsi prononce ...", "Aldus
      # uitgesproken ..."), everything left is names, dates and courtroom
      # composition. Nothing there is a section heading, and the initials of a
      # judge are indistinguishable from a Roman numeral: "V. SCHMITZ. M.-L.
      # WILLOT-THOMAS." was being rendered as collapsible section V. This guard
      # has to sit here because signature_mode is set further down, after this
      # chain, so it could never protect the chain itself.
      # Columnar layout - two or more tabs, or four or more spaces in a row -
      # is how a signature row is typeset ("GUERET          LEMAL") in courts
      # that never write "Ainsi prononce", so signature_mode never opens for
      # them. A section title is single-spaced text; the columns are the tell.
      # The gap after a leading numeral is exempt: "I.         OBJET ET
      # CONTEXTE" is a heading whose PDF simply put a wide tab-stop there.
      if signature_mode || stripped.sub(/\A[IVXLC]+\.\s+/, '').match?(/\t{2,}| {4,}/)
        current_para << escaped

      # Roman numeral sections: "I. Onderwerp van de beroepen", "II. Procédure", "III. Faits"
      elsif stripped.match?(/\A[IVXLC]+\.\s+\S/)
      ...
      # "Het Arbitragehof,", "Het Grondwettelijk Hof,", "wijst na beraad het volgende arrest :"
        ...
        flush_para.call
        # "Het Hof," and "wijst na beraad" are the court opening its reasoning:
        # the masthead is over, so the prose that follows is body, not intro.
        # "In zake :" is the opposite - it introduces the parties and belongs to
        # the masthead, whose line breaks must survive.
        intro_mode = false unless stripped.match?(/\AIn zake/i)
      ...
      # "PAR CES MOTIFS," / "OM DIE REDENEN," / "BESLUIT" - decision header
      elsif dispositif_line.call(stripped)
