...
module ParliamentaryHelper
  ...
  # Strips redundant "Doc ss 2355/010" prefixes since dossier/doc numbers
  # are already shown in badges.
  #
  # @param desc [String, nil] Raw description from substr(content, 1, 300)
  ...
  def clean_parliamentary_description(desc, target_language: nil)
    ...
    # Apply bilingual filter first if a target language is known
    lang = target_language || (I18n.locale == :nl ? 'nl' : 'fr')
    s = filter_bilingual_content(desc.to_s, lang)

    s = s.gsub(/\s+/, ' ').strip

    # Strip leading "Doc ss", "Docss", "pocs2", "poc", "oc" + number patterns
    s = s.sub(%r{\A(?:Doc\s*s{0,2}|poc\s*s?\d?|oc)\s+\d{1,5}/\d{1,5}\s*}i, '')

    # Strip leading dossier number pattern (e.g., "2355/010 ")
    s = s.sub(%r{\A\d{1,5}/\d{1,5}\s+}, '')

    # Strip leading "s2 " or "s " prefix
    s = s.sub(/\As\d?\s+/i, '')

    # Strip common headers
    s = s.sub(/\A(?:SESSION\s+EXTRAORDINAIRE\s+\d{4}|BUITENGEWONE\s+ZITTING\s+\d{4})\s*/i, '')

    # Strip OCR garbage patterns
    s = s.sub(/\ACn:\s*°\s*Tek\s+aangenomen[^\n]*\n?/i, '')
    s = s.sub(/\Aoor:\s+Wetsontwe?r?\.?\s*/i, '')

    # Strip spaced-out OCR header artifacts from vector-rendered PDFs
    # e.g., "K A M E R • 4 e Z I T T I N G S P E R I O D E 11 februari 2022 Belgische Kamer van volksvertegenwoordigers Zie: Doc 55..."
    s = s.sub(/[A-Z]\s(?:[A-Z]\s){3,}[A-Z].*?(?:Belgische\s+Kamer|Chambre\s+des|volksvertegenwoordigers|représentants)[^:]*(?:Zie:|Voir:)\s*/i, '')
    # Also strip simpler "DOC ss NNNNN KAMER • 4e ZITTING..." patterns
    s = s.sub(/\ADOC\s+\d{1,2}\s+\d{4,6}\s+\d{4}\s+K\s*A\s*M\s*E\s*R\s*[•·]\s*\d\s*e\s*Z\s*I\s*T\s*T\s*I\s*N\s*G[^Z]*?Zie:\s*/i, '')
  ...
  # Strips dossier number prefixes since they're shown elsewhere.
  #
  # @param title [String, nil] Raw title from DB
  # @param dossier [String, nil] Dossier number for fallback
  # @return [String] Cleaned title
  def clean_parliamentary_title_display(title, dossier = nil)
    return "Dossier #{dossier}" if title.blank? && dossier.present?
  ...
  def clean_jurisprudence_full_text(text, case_number: nil)
    ...
    lines.each_with_index do |line, idx|
      ...
      # Skip JuPortal PDF page markers: "X-18.860-6/30", "X-7080-1/3", "X-6623-1/4"
      # Format: X-{case_ref}-{page}/{total}  (standalone line)
      next if stripped.match?(%r{\AX-[\d.]+-\d+/\d+\z})
    ...
    result.each_line do |line|
      ...
      next if stripped.match?(/\A(?:Print deze pagina|Afdrukformaat|Nieuwe JUPORTAL|Sluit Tab)\z/i)
  ...
  def strip_wetgevingstechnische_nota(text)
    ...
    nota_markers = [
      ...
      /division\s+Af(?:f|ﬁ)(?:ir|aires)\s+juridiques/i
    ]
