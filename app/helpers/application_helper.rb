...
module ApplicationHelper
  # Include Pagy's frontend helpers for pagination
  ...
  def article_id_from_toc_line(line)
    ...
    normalized.present? ? "art-#{normalized}" : nil
  end
  ...
  def article_token_pattern
    %r{\b((?:[IVXLCDM]+\.)?[[:space:]\u00A0\u202F]*\d+(?:/\d+)?(?:[.:]\d+)*[a-z]*|[A-Za-z]\.??\d+(?:[.:]\d+)*|\d+(?:/\d+)?(?:[.:]\d+)*[a-z]*|[A-Za-z]+)\.?}ix
  end
  ...
  def article_number_pattern
    @article_number_pattern ||= %r{
      ...
      (?:
        (?:[IVXLCDM]+)\.?(?:[[:space:]\u00A0\u202F])*\d+(?:/\d+)?(?:[.:]\d+)*[a-z]*  # Roman + arabic, e.g., IV. 54/5 or XX. 1 (supports colon)
        |
        [A-Za-z]\.??\d+(?:[.:]\d+)*                              # A.1 or A1.2 (supports colon)
        |
        \d+(?:/\d+)?(?:[.:]\d+)*[a-z]*                         # 28/1, 2.3, 4bis (supports colon like 1:34)
        # Optional special designations (check most specific first to avoid backtracking)
        (?:_BRUSSELS_HOOFDSTEDELIJK_GEWEST|_BRUSSELSE_GEWEST|_WAALS_GEWEST|_VLAAMS_GEWEST|[\s\u00A0\u202F]+BRUSSELS(?:E)?[\s\u00A0\u202F]+GEWEST|[\s\u00A0\u202F]+WAALS[\s\u00A0\u202F]+GEWEST|[\s\u00A0\u202F]+VLAAMS[\s\u00A0\u202F]+GEWEST|[\s\u00A0\u202F]+TOEKOMSTIG[\s\u00A0\u202F]+RECHT|[\s\u00A0\u202F]+DROIT[\s\u00A0\u202F]+FUTUR)?
        |
  ...
  def replace_literal_domain_tags(text)
    ...
    text.gsub(literal_pattern) do |match|
      ...
      if is_closing
        ...
        if content
          # For tags with content (e.g., <KB 2007-04-25/32>), render as plain styled text
          # Don't include < > brackets to keep it clean and avoid confusion with HTML
          %(<span class="domain-tag domain-#{tag}">#{tag.upcase} #{CGI.escapeHTML(content)}</span>)
        else
          # For empty tags like <KB>, just open the span (closing </KB> will close it)
  ...
  def replace_encoded_domain_tags(text)
    ...
    text.gsub!(open_enc) do |match|
      ...
      content = match[/#{tag}\s+([^&]+)/i, 1]
      if content
        # For tags with content, render as plain styled text (no brackets)
        %(<span class="domain-tag domain-#{tag}">#{tag.upcase} #{content}</span>)
      else
        # For empty tags like &lt;KB&gt;, just open the span (closing will close it)
        %(<span class="domain-tag domain-#{tag}">)
      end
    end
    text
  ...
  def domain_tag_names
    %w[opgeheven ingevoegd w l ord opnieuw kb hersteld dvr ar nota]
  end
  ...
  def effective_date_patterns
    [
      /(?:Inwerkingtreding|Entrée\s+en\s+vigueur)[^\d]*(\d{1,2}[\s-]\w+[\s-]\d{4})/i,
      %r{(?:Inwerkingtreding|Entrée\s+en\s+vigueur)[^\d]*(\d{1,2}[/\-.]\d{1,2}[/\-.]\d{2,4})}i,
