...
module ApplicationHelper
  ...
  def article_id_from_toc_line(line)
    ...
    return nil unless line.match(ANCHORED_ARTICLE_NUMBER_PATTERN)
  ...
  def dom_highlight_html(original, pattern)
    ...
    Rails.logger.warn("[Helper] Rendering failed: #{e.message}")
    nil
  end
  ...
  def highlight_search_term(text, term)
    ...
    # Match whole words only. Substring matching mangled every title
    # containing a short term: searching "De tijd" turned "20 december 2024
    # houdende tijdelijke aanvullende maatregelen" into "20 <de>cember 2024
    # hou<de>n<de> <tijd>elijke aanvullen<de> maatregelen", because each
    # <mark> is inline-block-ish and visually splits the word.
    pattern = /(?<![[:alnum:]])(#{words.join('|')})(?![[:alnum:]])/i
  ...
  # @note Content is pre-sanitized via escape_angle_brackets before formatting
  ...
  # ARTICLE_NUMBER_PATTERN and its anchored forms live in ReferencesHelper
  # (included above), so both helpers can reference them at load time.

  # Returns the article-number Regexp in extended mode for readability
  ...
  def abolished_article_header?(article_type_str, content_str)
    ...
    header_match = s.match(LEADING_WS_ARTICLE_NUMBER_PATTERN)
  ...
  # Ordinal prefixes used in Belgian legal structure headings.
  # E.g. "Eerste Afdeling", "TWEEDE HOOFDSTUK", "Troisième Section"
  ORDINAL_PREFIX_PATTERN = '(?:(?:EERSTE|TWEEDE|DERDE|VIERDE|VIJFDE|ZESDE|ZEVENDE|ACHTSTE|NEGENDE|TIENDE|' \
                           'PREMIER|PREMI[ÈE]RE|DEUXI[ÈE]ME|TROISI[ÈE]ME|QUATRI[ÈE]ME|CINQUI[ÈE]ME|SIXI[ÈE]ME|SEPTI[ÈE]ME|HUITI[ÈE]ME|NEUVI[ÈE]ME|DIXI[ÈE]ME|' \
  ...
  # Handles ordinal prefixes like "Eerste Afdeling", "TWEEDE HOOFDSTUK"
  ...
  def section_heading_level(content)
    ...
    return 1 if text.match?(/\A#{ORDINAL_PREFIX_PATTERN}?(?:DEEL|PART|PARTIE)\b/)

    # Level 2: BOEK/LIVRE/BOOK
    return 2 if text.match?(/\A#{ORDINAL_PREFIX_PATTERN}?(?:BOEK|LIVRE|BOOK)\b/)

    # Level 3: TITEL/TITRE/TITLE
    return 3 if text.match?(/\A#{ORDINAL_PREFIX_PATTERN}?(?:TITEL|TITRE|TITLE)\b/)

    # Level 4: HOOFDSTUK/CHAPITRE/CHAPTER
    return 4 if text.match?(/\A#{ORDINAL_PREFIX_PATTERN}?(?:HOOFDSTUK|CHAPITRE|CHAPTER)\b/)

    # Level 5: AFDELING/SECTION
    return 5 if text.match?(/\A#{ORDINAL_PREFIX_PATTERN}?(?:AFDELING|SECTION)\b/)

    # Level 6: ONDERAFDELING/SOUS-SECTION
    return 6 if text.match?(/\A#{ORDINAL_PREFIX_PATTERN}?(?:ONDERAFDELING|SOUS-SECTION|SUBSECTION)\b/)

    # Level 7: Other named sections
    return 7 if text.match?(/\A#{ORDINAL_PREFIX_PATTERN}?(?:PARAGRAAF|PARAGRAPHE|PARAGRAPH)\b/)
