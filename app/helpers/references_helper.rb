...
module ReferencesHelper
  ...
  def build_section_heading_html(processed_content, heading_id, references_text)
    content_tag(:section, class: 'section-heading group mt-2 md:mt-3 mb-2 md:mb-3',
                          'data-section-heading': heading_id) do
      safe_join([
  ...
  def render_abolished_section_heading(content, _references = {})
    ...
    if abolished_raw.present?
      ...
      if refs.present?
        ...
        link_options = {
          title: t('articles.view_legislation'),
          target: '_blank',
  ...
  # Highlights abolished markers like <Opgeheven ...> or <Abrogé ...> in red while preserving brackets
  # Also highlights plain text abolished phrases for complete coverage
  def highlight_abolished_markers(html)
    return ''.html_safe if html.blank?

    str = html.to_s

    # Highlight angle-bracket abolished markers only (e.g., <Opgeheven ...>, &lt;Opgeheven ...&gt;)
    # Do NOT highlight plain text "wordt opgeheven" as it may be part of normal legislative text
    # describing that something else is being abolished
    highlighted = str.gsub(/((?:&lt;|<)\s*(?:Opgeheven|Abrogé)[^<>]*?(?:&gt;|>))/i) do |match|
      safe = match.start_with?('&lt;') ? match : ERB::Util.h(match)
      cls = [
        'text-red-700 dark:text-red-400',
        '[&_a]:text-red-700 dark:[&_a]:text-red-400'
      ].join(' ')
      prefix = %(<span class="#{cls}" data-skip-doc-links="true">)
      suffix = %(</span>)
      prefix + safe + suffix
    end
  ...
  def color_standalone_square_refs(text, reference_colors, show_colors = true)
    ...
    text.gsub(/\[(\d+)\](?![\d\]])/) do
      ...
      classes = ['reference', 'inline-block', 'whitespace-nowrap', color, 'font-medium'].compact.join(' ')
  ...
  def build_bracketed_ref_span(ref_num, inner, reference_colors, show_colors)
    color = show_colors ? color_for(reference_colors, ref_num) : ''
    left_marker  = "<span class='inline-block whitespace-nowrap'>[#{ref_num} </span>"
    right_marker = "<span class='inline-block whitespace-nowrap'>]#{ref_num}</span>"
    classes = ['reference', color, 'font-medium'].compact.join(' ')
    "<span class='#{classes}' data-controller='reference-highlight' data-ref-number='#{ref_num}' data-action='mouseenter->reference-highlight#highlight mouseleave->reference-highlight#unhighlight'>#{left_marker}#{inner}#{right_marker}</span>"
  end
  ...
  def build_parenthetical_ref_span(ref_num, ref_text, reference_colors, show_colors)
    color = show_colors ? color_for(reference_colors, ref_num) : ''
    marker = "<span class='inline-block whitespace-nowrap'>(#{ref_num})</span>"
    cls = ['reference', color, 'font-medium', 'hover:underline decoration-current hover:decoration-current',
           'cursor-pointer'].compact.join(' ')
    "<span class='#{cls}' data-controller='reference-highlight' data-ref-number='#{ref_num}' data-action='mouseenter->reference-highlight#highlight mouseleave->reference-highlight#unhighlight'>#{marker}#{ref_text}</span>"
  end
  ...
  def build_special_marker_span(marker, ref_text, show_colors)
    ...
    cls = ['reference', color, 'font-medium', 'hover:underline decoration-current hover:decoration-current',
           'cursor-pointer'].compact.join(' ')
    "<span class='#{cls}'>#{marker_span}#{ref_text}</span>"
  end
  ...
  def build_references_html(lines, doc_number_positions)
    lines.map do |line|
      process_single_reference_line(line, doc_number_positions)
    end.compact
  end

  def process_single_reference_line(line, doc_number_positions)
    trimmed = line.to_s.gsub(/\A(?:[[:space:]\u00A0\u202F]|&nbsp;)+|(?:[[:space:]\u00A0\u202F]|&nbsp;)+\z/, '')
    return nil if trimmed.match?(/\A(?:\[\d+\]|\(\d+\))\z/)

    ref_num = first_reference_number(line)
    ref_color = ref_color_class_for(ref_num)
    processed_line = build_linked_reference_line(line, ref_color, doc_number_positions) ||
                     process_reference_markers(line, {})

    build_reference_row_html(ref_num, ref_color, processed_line)
  end

  def build_references_container(references_html, anchor_id)
    content_tag(:div, class: 'references mt-3 not-prose', id: anchor_id) do
      safe_join([
  ...
  def find_lookup_for_line(line, doc_number_positions)
    ...
    if !lookup_entry && (rec = DocumentNumberLookup.find_by(document_number: doc_number))
      lookup_entry = { numac: rec.numac, language_id: rec.language_id }
  ...
  def build_reference_row_html(ref_num, ref_color, processed_line)
    if ref_num
  ...
  def build_row_with_marker(ref_num, ref_color, processed_line)
    reference_marker = content_tag(:span, "[#{ref_num}]",
                                   class: "#{ref_color} font-medium",
                                   data: {
                                     ref_target: ref_num,
                                     controller: 'reference-highlight',
                                     ref_number: ref_num,
                                     action: 'mouseenter->reference-highlight#highlightInline mouseleave->reference-highlight#unhighlightInline'
                                   })
    cleaned = processed_line.to_s.gsub(/\[\d+\]/, '').gsub(/\]\d+/, '')
    if cleaned.include?('<a ')
      inner_node = content_tag(:span, cleaned.html_safe, class: "reference #{ref_color}")
      return content_tag(:div, class: 'text-sm mb-1 font-mono') { reference_marker + inner_node }
    end
    ...
    reference_text = content_tag(:span, plain.html_safe, class: ref_color)
    content_tag(:div, class: 'text-sm mb-1 font-mono') { reference_marker + reference_text }
  end
  ...
  def link_abolished_content(cleaned_content, fallback_art_num = nil)
    ...
    link_to(safe_label.html_safe, url, class: abolished_anchor_classes,
                                       title: t('articles.view_legislation'), target: '_blank', rel: 'noopener noreferrer')
  end

  def find_document_lookup_for_abolished(cleaned_content)
    doc_number = cleaned_content[%r{(\d{4}-\d{2}-\d{2}/[A-Za-z0-9]+)}, 1]
    doc_number && DocumentNumberLookup.find_by(document_number: doc_number)
  end
  ...
  def load_missing_document_lookups(doc_numbers)
    ...
    DocumentNumberLookup.where(document_number: missing).find_each do |lookup|
      @_doc_lookup_cache[lookup.document_number] = { numac: lookup.numac, language_id: lookup.language_id }
    end
  end
  ...
  def link_metadata_tag_spans(text, lookups)
    text.gsub(%r{<span class="[^"]*\b(metadata-tag|nota-tag)\b[^"]*">([^<]+)</span>}i) do |span_html|
  ...
  def process_metadata_tag_span(span_html, match, lookups)
    ...
    link_to(span_html.html_safe, url, class: anchor_classes, title: t('articles.view_legislation'),
  ...
  def build_doc_number_link(html, original, lookup, doc_number, idx)
    ...
    link_to(doc_number, url, class: link_classes, title: t('articles.view_legislation'), target: '_blank', rel: 'noopener noreferrer')
  end
