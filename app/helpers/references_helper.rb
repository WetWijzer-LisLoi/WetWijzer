...
module ReferencesHelper
  ...
  def find_lookup_for_line(line, doc_number_positions)
    ...
    if !lookup_entry && (rec = DocumentNumberLookup.includes(content: :legislation).find_by(document_number: doc_number))
      law_title = rec.content&.legislation&.title
      lookup_entry = { numac: rec.numac, language_id: rec.language_id, title: law_title }
  ...
  def find_document_lookup_for_abolished(cleaned_content)
    ...
    rec = DocumentNumberLookup.includes(content: :legislation).find_by(document_number: doc_number)
    return nil unless rec

    law_title = rec.content&.legislation&.title
    { numac: rec.numac, language_id: rec.language_id, title: law_title }
  end
  ...
  def load_missing_document_lookups(doc_numbers)
    ...
    DocumentNumberLookup.where(document_number: missing).includes(content: :legislation).find_each do |lookup|
      law_title = lookup.content&.legislation&.title
