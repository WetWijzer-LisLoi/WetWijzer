...
module ReferencesHelper
  include References::DocumentLinking
  include References::TaxCrossLinking
  ...
  LIST_MARKER_COLOR = 'text-(--list-marker)'
  # SECTION_SYMBOL_COLOR removed - § markers render as plain text (no styling)
  SPECIAL_MARKER_COLOR = 'text-(--special-marker)'
  ...
  # ============================================================================
  # SECTION 1: REFERENCE EXTRACTION & COLORING (lines ~68-695)
  # Extracts [N ...] and (N ...) reference markers, assigns CSS colors
  # ============================================================================
  ...
  def any_articles_have_references?(articles)
    ...
    # For large laws, checking every article is wasteful. References in Belgian legislation
    # appear throughout the text if present at all. Sampling the first 50 articles is sufficient
    # to determine toggle button visibility, saving potentially thousands of regex scans.
    check_set = articles.size > 50 ? articles.first(50) : articles
    check_set.any? do |article|
  ...
  def apply_content_formatting(main_content)
    # Phase 1: Sanitize the raw DB text WITHOUT domain tag transformation.
    # Domain tags (<W>, <KB>, etc.) must stay as angle-bracket text during this
    # phase, because the reference marker regexes ([N ... ]N, (N)<...>) need
    # clean text without <span> wrappers to match correctly.
    sanitized = safe_db_content_without_domain_tags(main_content, :regular_article).to_s

    # Phase 2: Process reference markers on clean, sanitized text.
    highlighted = highlight_abolished_markers(sanitized)
    highlighted = highlight_modification_markers(highlighted)
    with_refs = process_reference_markers(highlighted, {})

    # Phase 3: NOW transform domain tags into styled <span> wrappers.
    # At this point, all bracket-based reference markers are already processed.
    with_domain_tags = transform_domain_specific_tags(with_refs.to_s)
    with_domain_tags = highlight_metadata_tags(with_domain_tags)

    format_line_breaks(with_domain_tags.html_safe)
  end
  ...
  def build_section_heading_h2(processed_content, heading_id)
    content_tag(:h2, id: heading_id,
      ...
      collapse_button = content_tag(:button, type: 'button',
                                             class: 'flex items-start gap-2 flex-1 min-w-0 text-left focus:outline-hidden focus-visible:ring-2 focus-visible:ring-(--accent-500) focus-visible:ring-offset-2',
                                             'data-action': 'click->collapse#toggle',
                                             'data-collapse-target': 'button',
                                             'aria-expanded': 'true') do
        chevron = content_tag(:svg, xmlns: 'http://www.w3.org/2000/svg',
                                    class: 'w-5 h-5 mt-0.5 shrink-0 transition-transform text-gray-500 dark:text-gray-400',
                                    viewBox: '0 0 20 20',
                                    fill: 'currentColor',
  ...
  def highlight_abolished_markers(html)
    ...
    # Fast-path: >95% of articles/TOC lines have no abolished markers.
    # Skip the expensive regex + document-number lookup when nothing to highlight.
    return str.html_safe unless str.match?(/opgeheven|abrog/i)
  ...
  def format_abolished_with_title(inner, cls)
    ...
    art_num = rest[/(?:art\.?|artikel|article)\s*(\d+[a-z]*)/i, 1]

    lookup = lookup_document_by_number(doc_number)
    return build_abolished_fallback(inner, cls).to_s unless lookup&.dig(:title).present?

    build_abolished_title_link(prefix, art_num, lookup, cls)
  end

  # -- abolished marker sub-helpers --

  def lookup_document_by_number(doc_number)
    return nil unless doc_number

    @_doc_lookup_cache ||= {}
    return @_doc_lookup_cache[doc_number] if @_doc_lookup_cache.key?(doc_number)
    ...
    @_doc_lookup_cache[doc_number] = { numac: rec.numac, language_id: rec.language_id, title: law&.title }
  rescue ActiveRecord::StatementInvalid
    nil
  end

  def build_abolished_title_link(prefix, art_num, lookup, cls)
    title = lookup[:title]
    label = art_num ? "#{prefix} art. #{art_num} van #{title}".strip : "#{prefix} #{title}".strip
    url = "/laws/#{lookup[:numac]}?language_id=#{lookup[:language_id]}"
  ...
  def build_abolished_fallback(inner, cls)
  ...
  # Formats list markers (°) for better visual distinction
  ...
  def color_parenthetical_refs(text, reference_colors, show_colors = true)
    ...
    text.gsub(/\((\d+)\)(<[^>]*>)/) do |match|
      ...
      next match if ref_text =~ %r{\A<\s*/?(?:td|tr|th|table|div|span|p|br|a|img|ul|ol|li|h[1-6])\b}i
  ...
  # ============================================================================
  # SECTION 2: REFERENCES SECTION RENDERING (lines ~699-1016)
  # Builds the visual references section below each article
  # ============================================================================
  ...
  def process_single_reference_line(line, doc_number_positions, ref_num, ref_color, is_continuation)
    # Strip [N] and ]N bracket markers before processing, since
    # build_row_with_marker adds its own marker separately.
    cleaned_line = line.to_s.gsub(/\[\d+\]/, '').gsub(/\]\d+/, '')

    # Escape raw <B ...> / <I ...> legislative references that the browser
    # would interpret as HTML tags (e.g. <B 11-10-1916, art. 1>)
    cleaned_line = cleaned_line.gsub(%r{<([biu])\s+(\d{2,4}[-/])([^>]*)>}i) do
      m = ::Regexp.last_match
      "&lt
#{m[1]} #{m[2]}#{m[3]}&gt
"
    ...
    # Only use build_linked_reference_line (which produces <a> links).
    # Do NOT call process_reference_markers here: it generates Stimulus
    # <span> tags that build_row_with_marker's non-link path will then
    # HTML-escape via escape_preserving_lt_gt, leaking data-action
    # attributes as visible text.
    processed_line = build_linked_reference_line(cleaned_line, ref_color, doc_number_positions) || cleaned_line
  ...
  def build_references_container(references_html, anchor_id)
    content_tag(:div, class: 'references references-section mt-3 not-prose', id: anchor_id) do
      safe_join([
                  content_tag(:h4, t('articles.changes'), class: 'font-semibold font-mono text-xs text-gray-800 dark:text-sky mb-2 border-0'),
  ...
  def build_row_with_marker(ref_num, ref_color, processed_line)
    ...
    if cleaned.include?('<a ')
      ...
      return %(<div class="font-mono text-xs mb-1" data-controller="reference-highlight" data-ref-number="#{ref_num}" data-action="mouseenter->reference-highlight#highlightInline mouseleave->reference-highlight#unhighlightInline">#{reference_marker}#{inner_node}</div>).html_safe
    end
    ...
    %(<div class="font-mono text-xs mb-1" data-controller="reference-highlight" data-ref-number="#{ref_num}" data-action="mouseenter->reference-highlight#highlightInline mouseleave->reference-highlight#unhighlightInline">#{reference_marker}#{reference_text}</div>).html_safe
  end

  def build_row_continuation(processed_line, parent_ref_num, parent_ref_color)
    # Calculate indentation: [N] marker width + small gap
    # Use ch units since we use monospace font
    indent_ch = parent_ref_num.to_s.length + 3
    ...
    if cleaned.include?('<a ')
      ...
      return %(<div class="font-mono text-xs mb-1" style="margin-left: #{indent_ch}ch;" data-controller="reference-highlight" data-ref-number="#{parent_ref_num}" data-action="mouseenter->reference-highlight#highlightInline mouseleave->reference-highlight#unhighlightInline">#{inner_node}</div>).html_safe
    end

    plain = escape_preserving_lt_gt(cleaned)
    reference_text = %(<span class="#{parent_ref_color}">#{plain}</span>).html_safe
    %(<div class="font-mono text-xs mb-1" style="margin-left: #{indent_ch}ch;" data-controller="reference-highlight" data-ref-number="#{parent_ref_num}" data-action="mouseenter->reference-highlight#highlightInline mouseleave->reference-highlight#unhighlightInline">#{reference_text}</div>).html_safe
  end
  ...
  def build_row_without_marker(processed_line)
    ...
    if raw.include?('<a ')
      content_tag(:div, class: 'font-mono text-xs mb-1 text-gray-600 dark:text-gray-400') { raw.html_safe }
    else
      escaped = escape_preserving_lt_gt(raw)
      content_tag(:div, class: 'font-mono text-xs mb-1 text-gray-600 dark:text-gray-400') { escaped.html_safe }
    end
  end
  ...
  ARTICLE_NUMBER_PREFIX_WS = '[[:space:]\u00A0\u202F\u200B\u200C\u200D\u2060\uFEFF]'
  # Compiled once: see ARTICLE_NUMBER_PATTERN for why.
  # Compiled once per process. It used to be memoised per view instance, and
  # every caller then interpolated .source into a fresh regex literal, which
  # Ruby recompiles on each evaluation unless the literal carries /o. This
  # 30-line pattern takes about a millisecond to compile, and
  # article_id_from_toc_line alone did it 1,223 times per render of a
  # 1,222-article law: 1.6 s of a 4.7 s cold render. The anchored forms the
  # callers need are constants too.
  ...
  ANCHORED_ARTICLE_NUMBER_PATTERN = /\A#{ARTICLE_NUMBER_PATTERN.source}/ix
  LEADING_WS_ARTICLE_NUMBER_PATTERN = /\A[[:space:]\u00A0\u202F]*#{ARTICLE_NUMBER_PATTERN.source}/ix

  # Leading article number wrapped in <b>/<strong>/<span>, with the trailing dot
  # outside the tag (DOT_OUTSIDE) or with everything inside the tag. Compiled
  # once: composing them per call recompiled the 30-line article-number pattern
  # for every article that reached safe_db_content.
  TAGGED_ARTICLE_NUMBER_DOT_OUTSIDE_PATTERN = %r{
    \A[[:space:]\u00A0\u202F]*
    <(?:b|strong|span)\b[^>]*>
      [[:space:]\u00A0\u202F]*
  ...
  TAGGED_ARTICLE_NUMBER_PATTERN = %r{
    \A[[:space:]\u00A0\u202F]*
    <(?:b|strong|span)\b[^>]*>
      [[:space:]\u00A0\u202F]*
      (?ix:(#{ARTICLE_NUMBER_PATTERN.source}))
  ...
  ARTICLE_NUMBER_PREFIX_PATTERN =
    /\A#{ARTICLE_NUMBER_PREFIX_WS}*(#{ARTICLE_NUMBER_PATTERN.source})(?:#{ARTICLE_NUMBER_PREFIX_WS}*)/ix

  def match_article_number_prefix(text)
    text.match(ARTICLE_NUMBER_PREFIX_PATTERN)
  end

  # ============================================================================
  # SECTION 3: ABOLISHED CONTENT RENDERING
  #   render_abolished_article, render_abolished_law, link_abolished_content
  # ============================================================================
  ...
  def wrap_abolished_article(article_number, content_html)
    content_tag(:div, class: 'abolished-article my-2') do
      parts = []
      if article_number.present?
        cleaned_article_number = article_number.sub(/[[:space:]\u00A0\u202F]+$/, '')
        parts << safe_join([article_number_tag(cleaned_article_number, class: 'text-gray-900 dark:text-gray-100'), content_html], ' ')
      else
        parts << content_html
      end
  ...
  def justel_archive_link_tag
    justel_url = defined?(@law) && @law ? @law.justel : nil
  ...
  # Returns full HTML or HTML when not matched.
  DVR_TAG_REGEX = /
    \A
    (.*?)                                   # before_text
  ...
  # ============================================================================
  # SECTION 4: ARTICLE PATTERN PROCESSING (lines ~1345-1496)
  # format_line_breaks, split_content_and_references, process_article_patterns
  # ============================================================================
  ...
  def process_article_patterns(content, content_type = :regular_article)
    ...
    if content_type == :regular_article
      ...
      elsif (m = text.match(TAGGED_ARTICLE_NUMBER_DOT_OUTSIDE_PATTERN))
      ...
      elsif (m = text.match(TAGGED_ARTICLE_NUMBER_PATTERN))
        num = m[1]
  ...
  def format_line_breaks(content)
    ...
    processed_content = process_article_patterns(content, :regular_article).to_s
    ...
    # Justel source data marks new paragraphs with \n followed by 2+ leading spaces.
    # After safe_db_content collapses multiple spaces to one, this becomes \n + single space.
    # Treat this as a paragraph indent (the standard Belgian legislation paragraph break).
    processed_content = processed_content.gsub("\n ", '<br/><span class="paragraph-indent"></span>')
    processed_content = processed_content.gsub("\n", '<br/>')
  ...
  # ============================================================================
  # SECTION 5: DOCUMENT LINKING & METADATA (lines ~1497-1856)
  # → Extracted to References::DocumentLinking
  #   lookup_document_references, apply_document_links, link_document_numbers
  # ============================================================================
