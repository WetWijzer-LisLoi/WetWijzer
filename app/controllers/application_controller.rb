...
class ApplicationController < ActionController::Base
  ...
  # Gate for Pro-only features (jurisprudence, etc.)
  def require_pro_subscription
    unless logged_in?
      # Return 410 Gone to search engine crawlers so they deindex gated pages
      if request.user_agent&.match?(/Googlebot|bingbot|Slurp|DuckDuckBot|Baiduspider|YandexBot/i)
        head :gone
        return
      end
      store_location
    ...
    redirect_to subscription_path, alert: t('subscriptions.pro_required', default: 'This feature requires a WetWijzer Pro subscription.')
  end
