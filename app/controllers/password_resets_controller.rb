  ...
  def create
    ...
    if user
      if EmailOutboxEntry.active_for_delivery?
        # FBL-032: token and outbox row commit atomically on the accounts
        # database; the drain job owns delivery with bounded retry.
        AccountRecord.transaction do
          user.generate_reset_token!
          EmailOutboxEntry.enqueue!(user, 'password_reset')
        end
      else
        ...
        begin
          ...
          # No raw address in logs (FBL-032).
          Rails.logger.error("[PasswordReset] Failed to send reset email user_id=#{user.id}: #{e.class}")
        end
      end
    end
