...
class ParliamentaryController < ApplicationController
  ...
  def search_documents(query, filters, limit, offset)
    ...
    sql = "SELECT id, parliament, dossier_number, document_number, title, url, document_date, document_type, substr(content, 1, 800), legislature FROM documents #{where_clause} ORDER BY id DESC LIMIT ? OFFSET ?"
    params += [limit, offset]
  ...
  def recent_documents(limit, offset)
    ...
    chamber_db.execute(
      "SELECT id, parliament, dossier_number, document_number, title, url, document_date, document_type, substr(content, 1, 800), legislature FROM documents WHERE language = ? ORDER BY id #{order} LIMIT ? OFFSET ?",
      [lang, limit, offset]
    ).map do |row|
      { id: row[0], parliament: row[1], dossier_number: row[2], document_number: row[3], title: row[4], url: row[5], date: row[6],
        document_type: row[7], description: row[8], legislature: row[9] }
  ...
  def available_parliaments
    ...
    Rails.cache.fetch('parliamentary_available_parliaments', expires_in: 24.hours, race_condition_ttl: 30.seconds) do
  ...
  def available_legislatures
    Rails.cache.fetch('parliamentary_available_legislatures', expires_in: 24.hours, race_condition_ttl: 30.seconds) do
