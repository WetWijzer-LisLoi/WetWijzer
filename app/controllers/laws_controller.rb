...
class LawsController < ApplicationController
  ...
  def export_word
    ...
    html_content = <<~HTML
      ...
      <body>
        <h1>#{ERB::Util.html_escape(law_title)}</h1>
    HTML

    @articles.each do |article|
      if article.article_type == 'LNK'
        # Section heading
        heading_text = helpers.strip_tags(article.article_title || article.article_text)
        html_content += "<h2>#{ERB::Util.html_escape(heading_text)}</h2>\n"
      else
        ...
        # Extract text content from rendered HTML (like innerText in JS)
        # This preserves reference text like "[1]", "(1)", etc.
        doc = Nokogiri::HTML.fragment(rendered_html)
        article_text = doc.text.strip

        # Escape and convert newlines to HTML
        article_text_escaped = ERB::Util.html_escape(article_text)
        article_text_formatted = article_text_escaped.gsub(/\n\n+/, '</p><p>').gsub("\n", '<br>')
    ...
    # Generate filename
    filename = "#{law_title.parameterize.presence || params[:numac]}.doc"
