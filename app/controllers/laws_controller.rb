...
class LawsController < ApplicationController
  ...
  def related_sources
    ...
    lookup_numac = @is_fisconet && @law.respond_to?(:numac_real) && @law.numac_real.present? ? @law.numac_real : params[:numac]

    # Cache related sources HTML for 1 hour â€” the underlying jurisprudence LIKE query
  ...
  def load_related_data
    # Content record (no need to eager-load :legislation â€” @law is already set by set_law)
    ...
    @articles = classic_host? ? load_articles : []
  ...
  def load_related_parliamentary(numac = nil)
    ...
    if File.exist?(db_path)
      begin
        ...
        # Strategy: use pre-computed parliamentary_refs column â†’ numac match â†’ dossier summary
        ...
        if items.empty?
          ...
          # Extract Kamer (K:) refs â€” format "K:55-3213"
          ...
          kamer_refs.each do |ref|
            ...
            if dossier_total.positive?
              ...
              # No documents scraped yet â€” show dossier summary from dossiers table
    ...
    # 2. Vlaamse Parlement (vlaamse_codex.sqlite3) â€” decreten, besluiten, verslagen
