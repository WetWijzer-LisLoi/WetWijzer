...
module GeoChallenge
  ...
  def known_crawler?
    ...
    if verified
      ...
      Rails.logger.warn("GeoChallenge: SPOOFED crawler detected! IP=#{request.remote_ip} UA=#{request.user_agent.to_s.truncate(80)} - DNS verification failed")
    end
    verified
  end
  ...
  def skip_geo_challenge?
    ...
    # Admin subdomain (has its own auth, nginx doesn't set GeoIP headers)
    ...
    # Error pages (prevent redirect loops when errors occur on non-standard subdomains)
    return true if path.start_with?('/500', '/404', '/422')
