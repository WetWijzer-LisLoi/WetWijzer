...
module LawsFiltering
  ...
  def run_search_if_needed
    ...
    # Determine which sources to search. Large external databases are queried
    # only when the user supplied a source-specific filter or search term; an
    # empty Turbo-frame or crawler request must not scan them merely because
    # their source checkbox defaults to enabled.
    search_legislation = params[:source_legislation] != '0'
    search_jurisprudence =
      params[:source_jurisprudence] != '0' && jurisprudence_query_present?
    search_parliamentary =
      params[:source_parliamentary] != '0' && parliamentary_query_present?
    ...
    if search_legislation
      ...
      begin
        ...
        canonical_scope = Legislation.where(
          language_id: LawSearchService::CANONICAL_LANGUAGE_IDS
        )
        legislation_scope = if term.present?
                              canonical_scope.where(
                                'LOWER(title) LIKE ?',
  ...
  def search_jurisprudence_source(page: 1, per_page: 20, sort: 'date_desc')
    ...
    JurisprudenceSearchService.search(
      ...
      date_from: params[:juris_date_from],
      date_to: params[:juris_date_to],
      page: page,
      per_page: per_page,
      sort: sort
    )
  ...
  def search_parliamentary_source(page: 1, per_page: 20, sort: 'date_desc')
    ParliamentarySearchService.search(
      ...
      date_from: params[:parl_date_from],
      date_to: params[:parl_date_to],
      page: page,
      per_page: per_page,
      sort: sort
    )
  ...
  def jurisprudence_query_present?
    [
      params[:title],
      params[:juris_court],
      params[:juris_year],
      params[:juris_lang],
      params[:juris_subject],
      params[:juris_date_from],
      params[:juris_date_to]
    ].any?(&:present?)
  end

  def parliamentary_query_present?
    [
      params[:title],
      params[:parl_parliament],
      params[:parl_year],
      params[:parl_doc_type],
      params[:parl_lang],
      params[:parl_date_from],
      params[:parl_date_to]
    ].any?(&:present?)
  end
