...
module AuthRateLimiting
  ...
  # 2FA verification: 10 attempts per 15 minutes per IP. Without this the TOTP /
  # backup-code check could be brute-forced within the pending-2FA cookie window.
  def rate_limit_2fa!
    return unless ip_rate_limited?('auth:2fa', limit: 10, window: 15.minutes)
