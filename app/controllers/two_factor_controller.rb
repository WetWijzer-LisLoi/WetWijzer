...
class TwoFactorController < ApplicationController
  ...
  before_action :require_authentication, except: %i[challenge verify]
  before_action :rate_limit_2fa!, only: [:verify]
  ...
  def setup
    ...
    cookies.signed[:pending_otp_secret] = { value: @otp_secret, expires: 10.minutes.from_now, httponly: true, secure: Rails.env.production?, same_site: :lax }
  ...
  def disable
    ...
    # Require password re-verification before disabling 2FA
    unless @user.authenticate(params[:password])
      redirect_to account_path, alert: t('account.wrong_password')
      return
    end
  ...
  def challenge
    return if cookies.signed[:pending_2fa_user_id]
  ...
  def complete_sign_in(user)
    ...
    # 'login' activity. A 2FA login is attributed HERE, at the completed
    # challenge - the password step wrote nothing.
    record_successful_sign_in!(user, remember: remember)
    ...
    if cookies.signed[:pending_pro].present?
      ...
      redirect_to checkout_path(tier: 'pro', interval: interval), notice: t('auth.signed_in')
    else
