...
export default class extends Controller {
  ...
  connect() {
    ...
    // Only horizontal peek - vertical centering is handled via computed pixel top
    this.collapsedDesktopTransform = `translate3d(calc(-100% + ${this.peekVisiblePx}px), 0, 0)`;
    ...
    if (this.hasPanelTarget) {
      try {
        ...
        if (isDesktop) {
          ...
          // Use computed pixel center to avoid sub-pixel jitter from 50% + translateY(-50%)
          this.panelTarget.style.top = this.computeCenterTopPx() + 'px';
          ...
          // Disable transition when collapsed to prevent scroll jitter
          this.panelTarget.style.transition = 'none';
          // Only horizontal peek - vertical centering already handled by top
          this.panelTarget.style.transform = this.collapsedDesktopTransform;
    ...
    // Desktop: close sidebar when clicking outside the panel
    try {
      this.handleDocumentClick = (e) => {
        ...
        if (e.target && (e.target === this.panelTarget || (e.target.closest && e.target.closest('[data-sidebar-toggle-target="panel"]')))) return;
        // Don't close if clicking a toggle button
        if (e.target && e.target.closest && e.target.closest('[data-sidebar-toggle-target="button"]')) return;
        // Don't close if clicking inside the TOC card (which may float over content)
        if (e.target && e.target.closest && e.target.closest('#toc-card-sidebar')) return;
        this.hideDesktopPanel();
      };
      document.addEventListener('mousedown', this.handleDocumentClick, { capture: true });
    } catch (_) {}
  ...
  disconnect() {
    ...
    try { if (this.handleDocumentClick) document.removeEventListener('mousedown', this.handleDocumentClick, { capture: true }); } catch (_) { /* no-op */ }
  ...
  onMediaChange(e) {
    ...
    if (e && e.matches) {
      ...
      try {
        this.panelTarget.style.position = 'fixed';
        // Use computed pixel center to avoid sub-pixel jitter
        this.panelTarget.style.top = this.computeCenterTopPx() + 'px';
        this.panelTarget.style.bottom = 'auto';  // Override class-based bottom: 0
        this.panelTarget.style.left = '0px';
        this.panelTarget.style.zIndex = '40';
  ...
  onTurboFrameLoad(e) {
    ...
    if (target && (target.id === 'law_articles')) {
      // Auto-open based on the server-side profile preference, only when articles load.
  ...
  // Check if auto-open is enabled via server-side profile preference
  isAutoOpenEnabled() {
...
showDesktopPanel() {
  ...
  const centerTop = this.computeCenterTopPx();
  ...
  panel.style.cursor = '';
  // Set the STARTING position (collapsed) with only horizontal peek
  panel.style.transform = this.collapsedDesktopTransform;
  ...
  // Step 5: Re-enable transition and trigger slide-in animation
...
hideDesktopPanel(skipTransition = false) {
  ...
  panel.classList.remove(this.collapsedMobileClass);
  panel.classList.add(this.collapsedDesktopClass);
  
  // Animate to collapsed, then kill transition to prevent scroll jitter
  try {
    panel.style.position = 'fixed';
    panel.style.left = '0px';
    panel.style.top = centerTop + 'px';
    ...
    // Briefly enable transition for the slide-out animation
    panel.style.transition = 'transform 300ms ease-in-out';
    panel.style.transform = this.collapsedDesktopTransform;
    // After animation completes, kill transition to prevent scroll jitter
    setTimeout(() => {
      try { panel.style.transition = 'none'; } catch (_) {}
    }, 320);
  } catch (_) {}
  ...
  recomputeMeasurements() {
    try {
      ...
      if (expanded) {
        ...
        // Collapsed sliver: recenter using computed pixel top (no transition)
        this.panelTarget.style.transition = 'none';
        this.panelTarget.style.top = this.computeCenterTopPx() + 'px';
...
updateExpandedFixedPosition() {
  try {
    ...
    const centerTop = this.computeCenterTopPx();
    this.panelTarget.style.left = '0px';
    this.panelTarget.style.top = centerTop + 'px';
    this.panelTarget.style.bottom = 'auto';
    this.panelTarget.style.transform = 'translate3d(0, 0, 0)';
  } catch (_) {}
}

// Compute the pixel top value that vertically centers the panel in the viewport
computeCenterTopPx() {
  try {
    const panelHeight = this.hasPanelTarget ? this.panelTarget.offsetHeight || 400 : 400;
    return Math.round((window.innerHeight - panelHeight) / 2);
  } catch (_) {
    return Math.round(window.innerHeight / 2 - 200);
  }
}
}
