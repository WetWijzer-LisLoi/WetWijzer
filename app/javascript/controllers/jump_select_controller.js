...
export default class extends Controller {
  ...
  positionSimpleFixed() {
    ...
    // Detect if inside the mobile bottom bar – use left-align instead of right-align
    const inBottomBar = btn.closest('#mobile-bottom-bar') !== null;
    ...
    if (inBottomBar) {
      // Left-align with button's left edge
      menu.style.left = `${Math.max(safe, rect.left)}px`;
    ...
    try { console.debug('jump-select: simple-fixed positioned', { rect, inBottomBar, top, placed, menuHeight, maxHeight }); } catch (_) {}
  }
}
