...
export default class extends Controller {
  ...
  refreshAfterFrameLoad() {
    ...
    try {
      this.cleanup()
      this.initialized = false
      // Defer slightly to allow the DOM to settle
      setTimeout(() => { this.initialize(); this.computeSectionTops() }, 50)
      setTimeout(() => this.syncFollowCheckbox(), 150)
      // After the frame content is in the DOM, try to activate and scroll to the current hash
      setTimeout(() => {
        this.activateByHash(window.location.hash)
