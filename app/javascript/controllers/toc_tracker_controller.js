...
export default class extends Controller {
  ...
  /**
   * Diagnostics: debug mode is only enabled via console command
   * Enable with: window._wwTocDebug = true
   */
  isDebugEnabled() {
    return window._wwTocDebug === true
  }
  ...
  /**
   * Load persisted enabled state from server-side profile
   * @return {boolean|null} true/false if present, otherwise null
   */
  loadEnabledFromStorage() {
    const val = prefs.get('toc_follow_enabled', null)
    return val
  }

  /**
   * Save enabled state to server-side profile
   * @param {boolean} flag
   */
  saveEnabledToStorage(flag) {
    prefs.set('toc_follow_enabled', flag)
  }
  ...
  initialize(skipInitialActive = false) {
    ...
    try {
      ...
      // Skip initial section activation during frame-load reinit to avoid
      // resetting to sections[0] while the user is scrolling
      if (!skipInitialActive) {
  ...
  initializeSections() {
    ...
    for (let i = 0; i < tocLinks.length; i++) {
      ...
      try {
        ...
        for (const c of candidates) {
          ...
          // Universal fallback: prefix-match for IDs with extended suffixes
          // Handles all types: section-afdeling-2 → section-afdeling-2-opgeheven-...,
          // art-6-4-4 → art-6-4-4-vlaams_gewest, section-hoofdstuk-i → section-hoofdstuk-i-doelstellingen-..., etc.
          if (!el) {
            el = document.querySelector(`[id^="${c}-"]`)
          }
          if (el) {
    ...
    // Build element → section Map for O(1) lookups in handleIntersection (Story 2.5)
    this._elementToSection = new Map(sections.map(s => [s.element, s]))
  ...
  handleTocClick(event) {
    ...
    let el = document.getElementById(preferredId) || document.getElementById(fallbackId)
    // Universal prefix-match: handles all ID patterns with extended suffixes
    if (!el) {
      el = document.querySelector(`[id^="${preferredId}-"]`)
    }
    ...
    if (el) {
      ...
      // Target not found — prevent browser from jumping to top of page
      event.preventDefault()
    }
  ...
  handleIntersection(entries) {
    ...
    entries.forEach(entry => {
      const section = this._elementToSection?.get(entry.target)
      if (section) {
  ...
  updateActiveLink(newActiveLink) {
    ...
    if (newActiveLink) {
      ...
      // Debounce the TOC auto-scroll so rapid section changes during fast
      // page scrolling don't cause the sidebar to visually jump around.
      // The highlight updates instantly; only the scroll is batched.
      if (this._tocScrollTimer) clearTimeout(this._tocScrollTimer)
  ...
  addActiveState(link) {
    ...
    // Note: removeActiveState for the previous link is already called by updateActiveLink()
    // No need to iterate all linkTargets (O(n) on large law pages with 2000+ sections)
  ...
  scrollToActiveLink(activeLink) {
    ...
    if (linkRect.top < containerRect.top || linkRect.bottom > containerRect.bottom) {
      ...
      // Use 'smooth' since the 200ms debounce in updateActiveLink prevents
      // competing scroll animations during fast page scrolling
  ...
  refreshAfterFrameLoad() {
    if (!this.enabledValue) return
    // Debounce rapid frame loads — only one reinit at a time
    if (this._refreshing) return
    this._refreshing = true
    try {
      // Preserve current state before cleanup so we don't jump back to start
      const savedActiveLink = this.currentActiveLink
      const savedActiveLinkTarget = savedActiveLink?.getAttribute('data-target') ||
        (savedActiveLink?.getAttribute('href') || '').replace(/^.*#/, '')

      this.cleanup()
      this.initialized = false
      // Single deferred initialization after DOM settles
      setTimeout(() => {
        this._refreshing = false
        // Pass skipInitialActive=true so initialize() doesn't reset to sections[0]
        this.initialize(true)
        this.computeSectionTops()
        this.syncFollowCheckbox()

        // If there's a URL hash, honour it; otherwise restore the previously active section
        const hash = window.location.hash
        if (hash) {
          this.activateByHash(hash)
          this.scrollToCurrentHash()
        } else if (savedActiveLinkTarget) {
          // Re-activate the section that was active before the frame reload
          this.activateByHash(`#${savedActiveLinkTarget}`)
        } else {
          // No hash and no saved section: fall back to scroll-position based activation
          this.setInitialActiveSection()
        }
