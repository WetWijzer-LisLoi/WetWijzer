...
export default class extends Controller {
  ...
  connect() {
    ...
    // Begin observing immediately on desktop so auto-open works when Tekst is scrolled into view
    this.observeIfNeeded();
    // Avoid immediate auto-open before user scrolls
    try {
      const isDesktop = this.desktopMedia && this.desktopMedia.matches;
      if (isDesktop && this.didUserScroll) this.evaluateTriggerVisibility();
    } catch (_) {}
    ...
    // If we loaded on an article anchor on desktop, open immediately
    try {
      const isDesktop = this.desktopMedia && this.desktopMedia.matches;
      if (isDesktop && this.hashTargetsArticle() && !this.autoOpenConsumed && this.hasEligibleSectionHeading()) {
  ...
  // Find the main content container that should avoid overlapping with the sidebar
  findContentEl() {
    try {
      // Narrow query to immediate layout container: <div class="flex ..."><div class="flex-1 ..."></div></div>
      const flex = this.element.querySelector(':scope > .flex');
      if (!flex) return null;
      // cache the flex parent for gap calculations
      this._flexEl = flex;
      // Prefer the specific main column used on the law show page
      const specific = flex.querySelector('.flex-1.min-w-0.space-y-6');
      if (specific) return specific;
      // Fallback to first flex-1 child
      return flex.querySelector('.flex-1');
    } catch (_) {
      return null;
    }
  }

  // Determine the horizontal gap between sidebar and content on desktop (lg:flex-row uses column-gap)
  flexGapPx() {
    try {
      const el = this._flexEl || this.element.querySelector(':scope > .flex');
      if (!el) return 0;
      const cs = window.getComputedStyle(el);
      // When flex-direction is row (desktop), use column-gap; otherwise row-gap is irrelevant for horizontal offset
      const dir = cs.flexDirection || 'row';
      if (dir.includes('row')) {
        const colGap = cs.columnGap || cs.gap || '0px';
  ...
  // Apply or clear a left offset on the content to avoid visual overlap with a fixed sidebar
  applyContentOffset(pxOrEmpty, addGap = true) {
    if (!this._contentEl) this._contentEl = this.findContentEl();
    if (!this._contentEl) return;
    try {
      if (pxOrEmpty === '' || pxOrEmpty == null) {
        this._contentEl.style.marginLeft = '';
      } else if (typeof pxOrEmpty === 'number') {
        const gap = addGap ? this.flexGapPx() : 0;
        this._contentEl.style.marginLeft = `${pxOrEmpty + gap}px`;
      } else {
        this._contentEl.style.marginLeft = pxOrEmpty;
      }
    } catch (_) {}
  }

  // Note: layout padding is no longer applied; flex/sticky layout handles spacing
  ...
  disconnect() {
    ...
    // Clear any content offset when disconnecting
    try { this.applyContentOffset(''); } catch (_) {}
  }
  ...
  onMediaChange(e) {
    ...
    if (e && e.matches) {
      ...
      // Clear any content offset used by expanded-fixed desktop state
      try { this.applyContentOffset(''); } catch (_) {}
  ...
  onTurboFrameLoad(e) {
    if (!this.hasTriggerTarget) return;
    // Our trigger is a stable wrapper, not the frame itself. Re-observe when the articles frame updates.
    const target = e && e.target;
    if (target && (target.id === 'law_articles')) {
      this.observeIfNeeded();
      // Evaluate after content loads only if user scrolled
      try {
        const isDesktop = this.desktopMedia && this.desktopMedia.matches;
        if (isDesktop && this.didUserScroll) this.evaluateTriggerVisibility();
      } catch (_) {}
  ...
  onHashChange(e) {
    // On desktop, opening an article anchor should open the sidebar
    try {
      const isDesktop = this.desktopMedia && this.desktopMedia.matches;
      if (!isDesktop) return;
      // Don't auto-open if user manually touched sidebar
      if (this.userManuallyTouched) return;
      // Suppress auto-open if a recent internal click initiated this hash change
      try {
        if (typeof this.suppressHashOpenUntil === 'number' && Date.now() < this.suppressHashOpenUntil) return;
      } catch (_) {}
      if (this.hashTargetsArticle() && !this.autoOpenConsumed && this.hasEligibleSectionHeading()) {
        this.showDesktopPanel();
        this.openedOnce = true;
        this.autoOpenConsumed = true;
        this.sync();
  ...
  handleScrollEvent(_) {
    ...
    this.scrollTick = window.requestAnimationFrame(() => {
      ...
      this.evaluateTriggerVisibility();
...
showDesktopPanel() {
  ...
  this.clearPeek();
  this.panelTarget.classList.add('translate-x-0');
  // Expanded on desktop: switch to fixed and center vertically while scrolling
  try {
    this.panelTarget.style.transform = '';
    this.panelTarget.style.position = 'fixed';
    this.panelTarget.style.zIndex = '40';
    this.panelTarget.style.pointerEvents = '';
    this.panelTarget.style.cursor = '';
    // Preserve current sliver top on the first expand to avoid a vertical jump
    this._lockTopOnce = this.readCurrentTop();
    this.updateExpandedFixedPosition();
    // After the first paint of expanded state, clear the one-time lock and recenter precisely
    try {
      if (this._postExpandRaf) { cancelAnimationFrame(this._postExpandRaf); this._postExpandRaf = null; }
    } catch (_) {}
    this._postExpandRaf = requestAnimationFrame(() => {
      this._postExpandRaf = null;
      try { this._lockTopOnce = undefined; this.updateExpandedFixedPosition(); } catch (_) {}
    });
  } catch (_) {}
...
hideDesktopPanel(skipTransition = false) {
  ...
  try {
    ...
    // IMPORTANT: Keep using same vertical centering method (50% + translateY) to avoid diagonal animation
    // Only change the X translation for horizontal slide
    const rect = this.panelTarget.getBoundingClientRect?.() || { height: 0 };
    const panelH = Math.round(rect.height || 0);
    const centerTop = Math.max(0, Math.round((window.innerHeight - panelH) / 2));
    this.panelTarget.style.top = '50%';
    this.panelTarget.style.bottom = '';
    // Combine horizontal peek with vertical centering
    this.panelTarget.style.transform = `${this.collapsedDesktopTransform} translateY(-50%)`;
  ...
  try { this.updateSliverHandlePosition(); } catch (_) {}
  // Clear any content offset used by expanded-fixed state
  try { this.applyContentOffset(''); } catch (_) {}
...
updateExpandedFixedPosition() {
  try {
    ...
    // When expanded, push content to the right to avoid overlap
    const rect = this.panelTarget.getBoundingClientRect?.() || { width: 0 };
    const sidebarWidth = Math.round(rect.width || 0);
    this.applyContentOffset(sidebarWidth);
  } catch (_) {}
}
}
