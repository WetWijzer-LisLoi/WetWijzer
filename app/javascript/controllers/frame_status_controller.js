...
export default class extends Controller {
  ...
  connect() {
    ...
    this.element.addEventListener("turbo:fetch-request-error", this.onFrameError);
    // frame-missing fires when the response doesn't contain the expected <turbo-frame>
    this.element.addEventListener("turbo:frame-missing", this.onFrameError);
  ...
  disconnect() {
    ...
    this.element.removeEventListener("turbo:fetch-request-error", this.onFrameError);
    this.element.removeEventListener("turbo:frame-missing", this.onFrameError);
  }
  ...
  onFrameError() {
    // Frame load failed – stop the timer and show error state instead of staying stuck
    this._clearFailsafe();
    const elapsed = this._stopTimer();
    this._showErrorPill(elapsed);
  }
  ...
  // Failsafe: if neither load nor error fires within 30s, force-clear the loading pill
  _startFailsafe() {
    this._clearFailsafe();
    this._failsafeTimeout = setTimeout(() => {
      if (this._startTime) {
        // Still loading after 30s – treat as error
        this.onFrameError();
      }
    }, 30000);
  }
  ...
  _showErrorPill(elapsedSec = null) {
    const target = this.mountTarget();
    this.clearExisting(target);
    ...
    icon.className = "inline-block w-2 h-2 rounded-full bg-red-500";

    const text = document.createElement("span");
    text.className = "flex-1";
    let msg = document.documentElement.lang === 'fr' ? 'Échec du chargement' : 'Laden mislukt';
    ...
    closeBtn.className = "ml-auto p-1 rounded transition-colors text-red-500 hover:text-red-700";
