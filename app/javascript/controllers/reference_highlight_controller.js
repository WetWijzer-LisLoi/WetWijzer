...
export default class extends Controller {
  // Check if highlighting is enabled via server-side profile
  isHighlightEnabled() {
    return prefs.get('reference_highlight', true)
  }
  ...
  highlight(event) {
    ...
    if (parentRef) {
      ...
      if (parentRefNumber && parentRefNumber !== refNumber) {
        ...
        const parentRefRows = articleContainer.querySelectorAll(`div[data-ref-number="${parentRefNumber}"]`)
    ...
    const refRows = articleContainer.querySelectorAll(`div[data-ref-number="${refNumber}"]`)
    refRows.forEach(row => {
