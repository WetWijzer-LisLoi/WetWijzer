...
export default class extends Controller {
    ...
    if (this.hasSourceSelectorValue) {
      try {
        ...
        if (node) {
          ...
          // After removing ref-markers, check .reference wrapper spans:
          // If a .reference span now only contains whitespace/ellipsis (deleted text),
          // remove it entirely. This handles [N ... ]N patterns where the brackets
          // were in separate ref-marker spans but the '...' was bare text between them.
          clone.querySelectorAll('.reference').forEach(ref => {
          ...
          // 1. Remove [N ... N] markers that indicate removed/repealed text
          // 2. Replace tabs with spaces
          // 3. Collapse multiple spaces to single space (within lines)
          // 4. Preserve paragraph breaks (single newlines between paragraphs)
          // 5. Remove empty lines but keep paragraph structure
          textToCopy = rawText
            .replace(/\[\d+\s*\.{3}\s*\d*\]/g, '')  // Remove [1 ... 1] style markers
            .replace(/\[\d+\s*…\s*\d*\]/g, '')      // Remove [1 … 1] with unicode ellipsis
            .replace(/\[\d+\s/g, '')                // Remove standalone opening [N markers
            .replace(/\]\d+/g, '')                   // Remove standalone closing ]N markers
            .replace(/\[\d+\]/g, '')                 // Remove [N] markers
            .replace(/(?:^|\s)\.{3}(?:\s|$)/gm, ' ') // Remove orphaned '...' from stripped markers
            .replace(/(?:^|\s)…(?:\s|$)/gm, ' ')     // Remove orphaned '…' from stripped markers
  ...
  copyAllArticlesCitation(event) {
    ...
    // Also removes [N ... N] markers that indicate removed/repealed text
    const cleanForCitation = (text) => {
      return text
        .replace(/\[\d+\s*\.{3}\s*\d*\]/g, '')  // Remove [1 ... 1] style markers (indicates removed text)
        .replace(/\[\d+\s*…\s*\d*\]/g, '')      // Remove [1 … 1] with unicode ellipsis
        .replace(/\[\d+\s/g, '')                // Remove standalone opening [N markers
        .replace(/\]\d+/g, '')                   // Remove standalone closing ]N markers
        .replace(/\[\d+\]/g, '')                 // Remove [N] markers
    ...
    const getCleanArticleText = (container) => {
      ...
      // Remove .reference wrappers that now only contain deleted-text indicators
      clone.querySelectorAll('.reference').forEach(ref => {
        const text = (ref.textContent || '').trim();
        if (!text || /^[\.…\s]+$/.test(text)) {
          ref.remove();
  ...
  copyCompareArticles(event) {
    ...
    const dateLabel = isDutch ? 'Geraadpleegd op' : 'Consulté le';
  ...
  copyCompareArticlesCitation(event) {
    ...
    // Helper to clean text for citation (remove references and removed-text markers)
    const cleanForCitation = (text) => {
      if (!text) return '-';
      return text
        .replace(/\[\d+\s*\.{3}\s*\d*\]/g, '')  // Remove [1 ... 1] style markers (indicates removed text)
        .replace(/\[\d+\s*…\s*\d*\]/g, '')      // Remove [1 … 1] with unicode ellipsis
        .replace(/\[\d+\s/g, '')                // Remove standalone opening markers [1 
        .replace(/\]\d+/g, '')                   // Remove standalone closing markers ]1
    ...
    const dateLabel = isDutch ? 'Geraadpleegd op' : 'Consulté le';
    textParts.push(`${dateLabel}: ${new Date().toLocaleDateString('nl-BE')}`);
    textParts.push(`\n${leftLang} / ${rightLang} (${isDutch ? 'citaat' : 'citation'})\n${'='.repeat(80)}\n`);
