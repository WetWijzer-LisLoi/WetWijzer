...
// Stores preferences in server-side profile (no localStorage)
export default class extends Controller {
  ...
  connect() {
    // Load preferences from the in-memory/server-side preference store.
    ...
    // Sync button states with the loaded preferences.
  ...
  loadPreferences() {
    // Read from server-side profile, defaulting to true if not set
    const storedColors = prefs.get('article_preferences.show_colors', null)
    const storedExdecs = prefs.get('article_preferences.show_exdecs', null)
    
    if (storedColors !== null) {
      this.showColorsValue = storedColors
    }
    
    if (storedExdecs !== null) {
      this.showExdecsValue = storedExdecs
    }
    
    const storedHighlight = prefs.get('article_preferences.show_highlight', null)
    if (storedHighlight !== null) {
      this.showHighlightValue = storedHighlight
    }
    
    const storedReferences = prefs.get('article_preferences.show_references', null)
    if (storedReferences !== null) {
      this.showReferencesValue = storedReferences
    }
  }
  ...
  toggleColors(event) {
    ...
    // Store in server-side profile
    prefs.set('article_preferences.show_colors', newValue)
  ...
  toggleExdecs(event) {
    ...
    // Store in server-side profile
    prefs.set('article_preferences.show_exdecs', newValue)
  ...
  syncColorsButton() {
    ...
    if (showColors) {
      ...
      button.innerHTML = `
        ...
        ${isDutch ? 'Verberg kleuren' : 'Masquer les couleurs'}
      `
    } else {
      ...
      button.innerHTML = `
        ...
        ${isDutch ? 'Toon kleuren' : 'Afficher les couleurs'}
      `
    }
  }
  ...
  syncHighlightButton() {
    ...
    if (showHighlight) {
      ...
      button.innerHTML = `
        ...
        ${isDutch ? 'Verberg markering' : 'Masquer le surlignage'}
      `
    } else {
      button.classList.remove('btn-toggle-active')
      button.classList.add('btn-toggle-inactive')
      button.innerHTML = `
        ...
        ${isDutch ? 'Toon markering' : 'Afficher le surlignage'}
      `
    }
  }
  ...
  toggleHighlight(event) {
    ...
    // Store in server-side profile
    prefs.set('article_preferences.show_highlight', newValue)
  ...
  toggleReferences(event) {
    ...
    // Store in server-side profile
    prefs.set('article_preferences.show_references', newValue)
  ...
  syncReferencesButton() {
    ...
    if (showReferences) {
      ...
      button.innerHTML = `
        ...
        ${isDutch ? 'Verberg wijzigingen' : 'Masquer les modifications'}
      `
    } else {
      button.classList.remove('btn-toggle-active')
      button.classList.add('btn-toggle-inactive')
      button.innerHTML = `
        ...
        ${isDutch ? 'Toon wijzigingen' : 'Afficher les modifications'}
      `
    }
