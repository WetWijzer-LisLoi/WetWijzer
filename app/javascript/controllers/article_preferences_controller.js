...
export default class extends Controller {
  ...
  toggleColors(event) {
    ...
    // PERFORMANCE OPTIMIZATION: Instead of toggling class on thousands of individual references,
    // toggle a single class on the document body. CSS descendant selectors will handle the rest.
    if (newValue) {
      // Show colors - remove the no-colors class from body
      document.body.classList.remove('ww-hide-ref-colors')
      console.log('[Color Toggle] Showing colors')
      console.log('[Color Toggle] Body classes:', document.body.className)
    } else {
      // Hide colors - add the no-colors class to body
      document.body.classList.add('ww-hide-ref-colors')
      console.log('[Color Toggle] Hiding colors')
      console.log('[Color Toggle] Body classes:', document.body.className)
      // Log a sample reference to check its computed style
      const sampleRef = document.querySelector('.reference')
      if (sampleRef) {
        console.log('[Color Toggle] Sample reference:', sampleRef.className)
        const computedColor = window.getComputedStyle(sampleRef).color
        console.log('[Color Toggle] Computed color:', computedColor)
      }
    }
