 ...
 *
 * Uses the Web Crypto API (SubtleCrypto) to encrypt/decrypt saved conversation
 * history in the browser. The server never receives that saved-history
 * plaintext or an unwrapped key; live prompts are still sent for answering.
 *
 * Architecture:
 ...
 * On unlock, the user's storage password re-derives the wrappingKey.
 */
...
export const ConversationCrypto = {
  ...
  async deriveWrappingKey(password, saltBase64) {
    ...
    const keyMaterial = await crypto.subtle.importKey(
      ...
      'PBKDF2',
      false,
      // PBKDF2 material may only derive keys/bits. The derived AES-GCM key
      // below receives the wrap/unwrap usages.
      ['deriveKey']
    )
