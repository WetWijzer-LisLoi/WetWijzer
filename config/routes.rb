...
Rails.application.routes.draw do
  ...
  resources :laws, param: :numac, only: %i[index show] do
    member do
      ...
      get :export_typst_compare
      get :export_odt_compare
    end
  end
  ...
  # PWA: manifest + offline fallback (locale/domain-aware, like the favicon).
  # The service worker itself is a static file (public/sw.js) to avoid Rails'
  # cross-origin-JavaScript guard.
  ...
  get 'jurisprudentie', to: redirect('/jurisprudence', status: 301) # SEO: Dutch spelling redirect
  ...
  get 'jurisprudence/*ecli/export_typst', to: 'jurisprudence#export_typst', as: 'export_typst_jurisprudence', format: false
  get 'jurisprudence/*ecli/export_odt', to: 'jurisprudence#export_odt', as: 'export_odt_jurisprudence', format: false
  ...
  post 'checkout/:tier/crypto', to: 'subscriptions#crypto_checkout', as: 'crypto_checkout'
  ...
  # Anonymous one-off donations (checkout creation remains POST + CSRF only)
  post 'donations', to: 'donations#create', as: 'donations'
  get 'donations/return', to: 'donations#complete', as: 'donation_return'
  get 'donations/cancel', to: 'donations#cancel', as: 'donation_cancel'

  # CSP violation reports (FBL-046 step 3; browsers POST with no CSRF/session)
  post 'csp-report', to: 'csp_reports#create'

  # Payment webhooks - always reachable (provider state is verified server-side)
  post 'webhooks/mollie', to: 'webhooks/mollie#create', as: 'webhooks_mollie'
  ...
  constraints admin_constraint do
    namespace :admin do
      ...
      get 'login', to: 'sessions#new', as: 'login'
      post 'login', to: 'sessions#create'
      # Second factor of the ADMIN_MFA_SESSIONS flow (FBL-040)
      post 'login/otp', to: 'sessions#verify_otp', as: 'login_otp'
      # Fresh-password confirmation before destructive actions (FBL-040)
      get 'reauth', to: 'sessions#reauth', as: 'reauth'
      post 'reauth', to: 'sessions#confirm_reauth'
      delete 'logout', to: 'sessions#destroy', as: 'logout'
  ...
  # API routes
  namespace :api do
    # Cache-proof deployment identity used by release-gating browser checks.
    get 'release', to: 'release#show'
    # Opaque full-provenance seal; v1 HMAC service authentication is mandatory.
    get 'quality/provenance', to: 'quality_provenance#show'
    # Release-gating browser probes. Credit state is session-owned; mutations
    # and ledger inspection require body-bound v1 HMAC authentication.
    get 'chatbot/quality/credit_state', to: 'chatbot_quality_probe#credit_state'
    post 'chatbot/quality/rate_limit/arm', to: 'chatbot_quality_probe#arm_rate_limit'
    post 'chatbot/quality/rate_limit/cleanup', to: 'chatbot_quality_probe#cleanup_rate_limit'
    post 'chatbot/quality/reservation_audit', to: 'chatbot_quality_probe#reservation_audit'
    ...
    # AI chatbot API
    # POST only: the JS streams SSE over fetch POST (no EventSource/GET use).
    # The former GET route was a CSRF hole — the endpoint deducts credits
    # with CSRF protection skipped, so a cross-site top-level GET (with
    # SameSite=Lax cookies attached) could drain a logged-in user's credits.
    # It also put the question (legal PII) in URLs and access logs.
    ...
    post 'chatbot/save', to: 'chatbot_saved_answers#save'
    get 'chatbot/saved', to: 'chatbot_saved_answers#saved'
    delete 'chatbot/saved/:id', to: 'chatbot_saved_answers#destroy_saved'
    ...
    # One singleton rating resource per answer, identified by its signed
    # token: every request replaces the complete score/reasons state, which is
    # why PUT. PATCH is accepted alongside it because every other update
    # endpoint in this API is PATCH and there is no PUT anywhere else in the
    # route table - so no proxy or client path in this deployment has ever
    # been exercised with PUT. Both verbs reach the same action.
    put 'chatbot/rating', to: 'chatbot_ratings#update'
    patch 'chatbot/rating', to: 'chatbot_ratings#update'

    # Conversation management (server-side history + zero-knowledge)
    get 'chatbot/conversations', to: 'chatbot_conversations#conversations'
    get 'chatbot/active_conversation', to: 'chatbot_conversations#active_conversation'
    # Static segments MUST precede the :token wildcards: with the old order,
    # DELETE conversations/consent matched destroy_conversation with
    # token='consent' and consent revocation always answered 404.
    post 'chatbot/conversations/consent', to: 'chatbot_conversations#grant_consent'
    delete 'chatbot/conversations/consent', to: 'chatbot_conversations#revoke_consent'
    post 'chatbot/conversations/import', to: 'chatbot_conversations#import_conversations'
    get 'chatbot/conversations/:token', to: 'chatbot_conversations#show_conversation'
    delete 'chatbot/conversations/:token', to: 'chatbot_conversations#destroy_conversation'
    patch 'chatbot/conversations/:token', to: 'chatbot_conversations#update_conversation'
    patch 'chatbot/conversations/:token/encrypted', to: 'chatbot_conversations#update_encrypted_payload'
    get 'chatbot/zk_key_material', to: 'chatbot_conversations#zk_key_material'
    ...
    # Partner app API removed 2026-08-08 when the two products were separated. The
    # controller, its client and the embedded-WebView middleware were deleted outright,
    # not archived: anything tracked in this repo deploys to the server.
  end
  ...
  get 'support', to: 'legal_pages#support', as: 'support_page'
