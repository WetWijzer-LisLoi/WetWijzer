...
Rails.application.configure do
  ...
  # Use file-based cache store (same as production) for restart persistence.
  config.cache_store = :file_store, Rails.root.join('tmp', 'cache'), {
    size: 256 * 1024 * 1024, # 256 MB disk budget
    expires_in: 7.days
  }

  # Durable background jobs via Solid Queue (see config/puma.rb `plugin
  # :solid_queue`, gated on SOLID_QUEUE_IN_PUMA). Jobs persist in the dedicated
  # storage/queue.sqlite3, surviving worker restarts (the old :async adapter
  # dropped queued jobs on every restart).
