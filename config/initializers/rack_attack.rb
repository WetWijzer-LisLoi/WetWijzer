...
module Rack
  class Attack
    ...
    # Always allow static JSON files in public folder (but not dynamic challenge endpoints)
    ...
    throttle('pagination/ip', limit: 30, period: 1.minute) do |req|
      req.ip if req.path == '/laws' && req.get? && req.params['page'].present?
    end
    ...
    # Throttle chatbot API - ONLY the /ask endpoint (LLM calls)
    # Other chatbot endpoints (conversations, settings, consent) are lightweight CRUD
    # and should NOT consume the rate limit budget (20 requests per minute per IP)
    throttle('chatbot/ip', limit: 20, period: 1.minute) do |req|
      req.ip if req.path == '/api/chatbot/ask' && req.post?
    end
