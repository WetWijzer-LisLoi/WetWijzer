...
module Rack
  class Attack
    ...
    # Throttle chatbot API (20 requests per minute per IP)
    # LLM calls are expensive (~$0.01-0.03 each) — prevent abuse while allowing normal use
    # A human user can't reasonably submit more than ~1 question every 3 seconds
    throttle('chatbot/ip', limit: 20, period: 1.minute) do |req|
      req.ip if req.path.start_with?('/api/chatbot')
    end
