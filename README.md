...
Belgian Official Gazette (ejustice.just.fgov.be)
    ...
    └── enrich_*.py                 → Embeddings, FTS indexes, metadata
         ...
         ▼
    SQLite Databases
    ├── legislation.db  (485K+ laws, 2.8M+ articles)
...
| Tier | Offered models | Base credits |
|------|----------------|:---:|
| ⚡ Smart | Mistral Small 4, Mistral Large 3, GPT-5 Mini | 1–2 |
| ⭐ Genius | GPT-5.6 Luna, Claude Haiku 4.5 | 4 |
| 🏆 Mastermind | GPT-5, Claude Sonnet 4.6, GPT-5.6 Terra | 6 |
| 🔮 Omniscient | GPT-5.6 Sol, Claude Opus 4.6 | 12–13 |

An optional reasoning depth can add 0–2 credits. The UI shows the exact total
before submission.
...
|------|-------|-----------|
| **Free** | €0 | 5 one-time starter credits on select Smart-tier models (currently Mistral Small 4 only), then purchased credits |
| **Pro** | €2.99/mo | 40 credits/mo, all 4 intelligence tiers, case law and parliamentary documents via the chatbot, 30% off credit packs |

### Credit Packs (Pay-As-You-Go)
| Pack | Credits | Price | Pro price (30% off) |
|------|---------|-------|---------------------|
| Starter | 10 | €2.99 | €2.09 |
| Standard | 20 | €4.99 | €3.49 |
| Power | 50 | €8.99 | €6.29 |

Source of truth: `CreditPurchase::PACKAGES` in `app/models/credit_purchase.rb`.

### Payment Processing
- **Mollie hosted checkout** for recurring subscriptions, one-time credit purchases, and direct anonymous donations
- **CoinGate** remains the optional cryptocurrency payment route
- Subscription checkout creates a Mollie customer and a first payment; after
  the resulting mandate is valid, a durable job provisions the monthly
  subscription
- Donations use fixed offered amounts and are one-off, reward-free payments:
  no account, Mollie customer, credits, access, invoice, or credit note is
  created
- Mollie's classic webhook is protected by a per-environment URL token and
  contains only a payment ID; WetWijzer always fetches current payment state
  from Mollie before granting credits or activating access
- Local `MolliePayment` and `MolliePaymentAdjustment` ledgers, stable
  idempotency keys, and retryable background jobs prevent duplicate
  fulfilment, invoices, refunds, and chargeback adjustments
- A lower chargeback total restores nothing on first observation. A delayed
  job performs a second fresh Mollie read and accepts only the same lower
  value; the non-unique adjustment index permits legitimate repeated
  chargeback/reversal cycles while compare-and-swap protects concurrency
- Scheduled recovery re-enqueues lost durable work every five minutes and
  polls a bounded rotating provider slice every fifteen minutes to repair
  missed callbacks
- **PEPPOL** e-invoicing compliance (mandatory for Belgian B2B since Jan 2026)
- **Octopus** accounting integration for invoice sync

Checkout creation routes are POST-only. Browser return URLs improve the user
experience but are not trusted as proof of payment; provider state is fetched
again on return and through the webhook.
